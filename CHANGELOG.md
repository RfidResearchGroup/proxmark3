...
## [unreleased][unreleased]
 - Added `COMPILING.txt` in `fpga-xc2s30` with an overview of how to compile the FPGA files (@d18c7db)
 - Updated the Spartan-2 `fpga_*.bit` files, includes the `hi_iso14443a.v` update (@d18c7db)
 - Changed the makefile and windows make.bat file in `fpga-xc2s30` to generate the XST script files on-the-fly (@d18c7db)
 - Deleted the `*.scr` files in `fpga-xc2s30` because they are now generated by the make scripts (@d18c7db)
 - Added minor orphaned change from `hi_iso14443a.v` in `fpga-xc3s100e` to `hi_iso14443a.v` in `fpga-xc2s30` (@d18c7db)
 - Moved historic `testbed_*.v` and `sim.tcl` files in `fpga-xc2s30` to `fpga-xc2s30/tests` (@d18c7db)
