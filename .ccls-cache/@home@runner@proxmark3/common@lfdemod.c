...
signal_t *getSignalProperties(void) { return &signalprop; }
...
static void printSignal(void) {
  ...
  prnt("  is Noise......%s",
       (signalprop.isnoise) ? _RED_("Yes") : _GREEN_("No"));
...
void computeSignalProperties(const uint8_t *samples, uint32_t size) {
  ...
  uint8_t low10 = 0.5 * (tmp[(int)(offset_size * 0.1)] +
                         tmp[(int)((offset_size - 1) * 0.1)]);
  uint8_t hi90 = 0.5 * (tmp[(int)(offset_size * 0.9)] +
                        tmp[(int)((offset_size - 1) * 0.9)]);
  uint32_t cnt = 0;
  for (uint32_t i = SIGNAL_IGNORE_FIRST_SAMPLES; i < size; i++) {

    if (samples[i] < signalprop.low)
      signalprop.low = samples[i];
    if (samples[i] > signalprop.high)
      signalprop.high = samples[i];
...
void removeSignalOffset(uint8_t *samples, uint32_t size) {
  ...
  uint8_t low10 = 0.5 * (tmp[(int)(offset_size * 0.05)] +
                         tmp[(int)((offset_size - 1) * 0.05)]);
  uint8_t hi90 = 0.5 * (tmp[(int)(offset_size * 0.95)] +
                        tmp[(int)((offset_size - 1) * 0.95)]);
  int32_t cnt = 0;
  for (uint32_t i = SIGNAL_IGNORE_FIRST_SAMPLES; i < size; i++) {
...
// get high and low values of a wave with passed in fuzz factor. also return
// noise test = 1 for passed or 0 for only noise void getHiLo(uint8_t *bits,
// size_t size, int *high, int *low, uint8_t fuzzHi, uint8_t fuzzLo) {
...
// pass bits to be tested in bits, length bits passed in bitLen, and parity type
// (even=0 | odd=1) in pType returns 1 if passed
bool parityTest(uint32_t bits, uint8_t bitLen, uint8_t pType) {
  return oddparity32(bits) ^ pType;
}

// takes a array of binary values, start position, length of bits per parity
// (includes parity bit - MAX 32),
//   Parity Type (1 for odd; 0 for even; 2 for Always 1's; 3 for Always 0's),
//   and binary Length (length to run)
size_t removeParity(uint8_t *bits, size_t startIdx, uint8_t pLen, uint8_t pType,
                    size_t bLen) {
  uint32_t parityWd = 0;
  size_t bitCnt = 0;
...
static size_t removeEm410xParity(uint8_t *bits, size_t startIdx, bool isLong,
                                 bool *validShort, bool *validShortExtended,
                                 bool *validLong) {
  uint32_t parityWd = 0;
  size_t bitCnt = 0;
...
// takes a array of binary values, length of bits per parity (includes parity
// bit), Parity Type (1 for odd; 0 for even; 2 Always 1's; 3 Always 0's), and
// binary Length (length to run) Make sure *dest is long enough to store
// original sourceLen + #_of_parities_to_be_added
size_t addParity(const uint8_t *src, uint8_t *dest, uint8_t sourceLen,
                 uint8_t pLen, uint8_t pType) {
  uint32_t parityWd = 0;
  size_t j = 0, bitCnt = 0;
...
// search for given preamble in given BitStream and return success = TRUE or
// fail = FALSE and startIndex and length
bool preambleSearch(uint8_t *bits, uint8_t *preamble, size_t pLen, size_t *size,
                    size_t *startIdx) {
  return preambleSearchEx(bits, preamble, pLen, size, startIdx, false);
}
// search for given preamble in given BitStream and return success=1 or fail=0
// and startIndex (where it was found) and length if not fineone fineone does
// not look for a repeating preamble for em4x05/4x69 sends preamble once, so
// look for it once in the first pLen bits (iceman) FINDONE,  only finds start
// index. NOT SIZE!.  I see Em410xDecode (lfdemod.c) uses SIZE to determine
// success
bool preambleSearchEx(uint8_t *bits, uint8_t *preamble, size_t pLen,
                      size_t *size, size_t *startIdx, bool findone) {
  ...
  for (size_t idx = 0; idx < *size - pLen; idx++) {
    if (memcmp(bits + idx, preamble, pLen) == 0) {
      ...
      if (foundCnt == 1) {
        if (g_debugMode >= 1)
          prnt("DEBUG: (preambleSearchEx) preamble found at %zu", idx);
        *startIdx = idx;
        if (findone)
          return true;
      }
      if (foundCnt == 2) {
        if (g_debugMode >= 1)
          prnt("DEBUG: (preambleSearchEx) preamble 2 found at %zu", idx);
        *size = idx - *startIdx;
        return true;
      }
    }
...
// find start of modulating data (for fsk and psk) in case of beginning noise or
// slow chip startup.
static size_t findModStart(const uint8_t *src, size_t size,
                           uint8_t expWaveSize) {
  size_t i = 0;
  ...
  if (g_debugMode == 2)
    prnt("DEBUG: threshold Count reached at index %zu, count: %u", i,
         thresholdCnt);
  return i;
}
...
// load wave counters
bool loadWaveCounters(uint8_t *samples, size_t size, int lowToLowWaveLen[],
                      int highToLowWaveLen[], int *waveCnt, int *skip,
                      int *minClk, int *high, int *low) {
  size_t i = 0;
  ...
  if (signalprop.isnoise) {
    if (g_debugMode == 2)
      prnt("DEBUG STT: just noise detected - quitting");
    return false;
  }
...
size_t pskFindFirstPhaseShift(const uint8_t *samples, size_t size,
                              uint8_t *curPhase, size_t waveStart, uint16_t fc,
                              uint16_t *fullWaveLen) {
  uint16_t loopCnt = (size + 3 < 4096)
                         ? size
  ...
  for (; i < loopCnt; i++) {
    // find peak // was "samples[i] + fc" but why?  must have been used to weed
    // out some wave error... removed..
    if (samples[i] < samples[i + 1] && samples[i + 1] >= samples[i + 2]) {
      waveEnd = i + 1;
      if (g_debugMode == 2)
        prnt("DEBUG PSK: waveEnd: %zu, waveStart: %zu", waveEnd, waveStart);
      waveLenCnt = waveEnd - waveStart;
      if (waveLenCnt > fc && waveStart > fc &&
          !(waveLenCnt >
            fc +
        ...
        *fullWaveLen = waveLenCnt;
        ...
        // (could cause inverting)
        if (lastAvgWaveVal > FSK_PSK_THRESHOLD)
          *curPhase ^= 1;
...
// amplify based on ask edge detection  -  not accurate enough to use all the
// time
...
void manchesterEncodeUint32(uint32_t data_in, uint8_t bitlen_in,
                            uint8_t *bits_out, uint16_t *index) {
...
// NOTE: bitstream must have triple the size of "size" available in memory to do
// the swap
int ManchesterEncode(uint8_t *bits, size_t size) {
  // allow up to 4096b out (means bits must be at least 2048+4096 to handle the
  // swap)
...
// loop 1024 samples,   if 250 of them is deemed maxed out,  we assume the wave
// is clipped.
bool DetectCleanAskWave(const uint8_t *dest, size_t size, uint8_t high,
                        uint8_t low) {
  ...
  for (size_t i = 160; i < loopEnd; i++) {
    ...
    if (dest[i] > low && dest[i] < high)
      ...
      cntPeaks++;
      // if (g_debugMode == 2) prnt("DEBUG DetectCleanAskWave: peaks (200) %u",
      // cntPeaks);
      if (cntPeaks > 200)
        return true;
    }
  ...
  if (allArePeaks == false) {
    if (g_debugMode == 2)
      prnt("DEBUG DetectCleanAskWave: peaks (200) %u", cntPeaks);
    if (cntPeaks > 200)
      return true;
  }
  return allArePeaks;
...
// **********************************************************************************************
// -------------------Clock / Bitrate Detection
// Section------------------------------------------
// **********************************************************************************************

// to help detect clocks on heavily clipped samples
// based on count of low to low
int DetectStrongAskClock(uint8_t *dest, size_t size, int high, int low,
                         int *clock) {
  size_t i = 100;
  size_t minClk = 768;
  ...
  uint16_t tmpclk[11][3] = {
      {8, 0, 0},   {16, 0, 0},  {32, 0, 0},  {40, 0, 0},
      {50, 0, 0},  {64, 0, 0},  {100, 0, 0}, {128, 0, 0},
      {256, 0, 0}, {272, 0, 0}, {384, 0, 0},
  };
  ...
  for (int j = 10; j > -1; j--) {
    if (g_debugMode == 2) {
      prnt("DEBUG, ASK,  clocks %u | hits %u | idx %u", tmpclk[j][0],
           tmpclk[j][1], tmpclk[j][2]);
    }
...
// not perfect especially with lower clocks or VERY good antennas (heavy wave
// clipping) maybe somehow adjust peak trimming value based on samples to fix?
// return start index of best starting position for that clock and return clock
// (by reference)
int DetectASKClock(uint8_t *dest, size_t size, int *clock, int maxErr) {
  ...
  if (size <= loopCnt + 60) {
    if (g_debugMode == 2)
      prnt("DEBUG DetectASKClock: not enough samples - aborting");
    return -1;
  }
  ...
  if (signalprop.isnoise) {
    if (g_debugMode == 2)
      prnt("DEBUG DetectASKClock: just noise detected - aborting");
    return -2;
  }
  ...
  if (!found_clk) {
    ...
    if (DetectCleanAskWave(dest, size, peak_hi, peak_low)) {
      ...
      if (g_debugMode == 2)
        prnt("DEBUG ASK: DetectASKClock Clean ASK Wave detected: clk %i, Best "
             "Starting Position: %i",
             *clock, idx);
  ...
  uint8_t clkCnt, tol;
  size_t j = 0;
  uint16_t bestErr[] = {1000, 1000, 1000, 1000, 1000,
  ...
  for (; clkCnt < num_clks; clkCnt++) {
    ...
    for (; j < loopCnt; j++) {
      ...
      for (i = 0; i < loopEnd; ++i) {
        ...
        } else if (dest[arrLoc - tol] >= peak_hi ||
                   dest[arrLoc - tol] <= peak_low) {
        } else if (dest[arrLoc + tol] >= peak_hi ||
                   dest[arrLoc + tol] <= peak_low) {
      ...
      // if we found no errors then we can stop here and a low clock (common
      // clocks)
      //  this is correct one - return this clock
      // if (g_debugMode == 2) prnt("DEBUG ASK: clk %d, err %d, startpos %d,
      // endpos %d", clk[clkCnt], errCnt, j, i);
  ...
  for (k = 1; k < num_clks; ++k) {
    ...
    // if (g_debugMode == 2) prnt("DEBUG ASK: clk %d, # Errors %d, Current Best
    // Clk %d, bestStart %d", clk[k], bestErr[k], clk[best], bestStart[best]);
  }
  ...
  if (chg == false) {
    if (g_debugMode == 2)
      prnt("DEBUG DetectASKClock: no good values detected - aborting");
    return -2;
  }
...
int DetectStrongNRZClk(const uint8_t *dest, size_t size, int peak, int low,
                       bool *strong) {
  ...
  *strong = false;
  ...
  if (g_debugMode == 2)
    prnt("DEBUG NRZ: detectstrongNRZclk smallest wave: %d", lowestTransition);
  // if less than 10% of the samples were not peaks (or 90% were peaks) then we
  // have a strong wave
...
int DetectNRZClock(uint8_t *dest, size_t size, int clock,
  ...
  if (signalprop.isnoise) {
    if (g_debugMode == 2)
      prnt("DEBUG DetectNZRClock: just noise detected - quitting");
    return 0;
  }
  ...
  bool strong = false;
  int lowestTransition =
      DetectStrongNRZClk(dest, size - 20, peak, low, &strong);
  if (strong)
    return lowestTransition;
  size_t ii;
  uint8_t clkCnt;
  ...
  for (i = 0; i < loopCnt; i++) {
    if (dest[i] >= peak || dest[i] <= low) {
      ...
      firstpeak = false;
      if (smplCnt > 0) {
        if (minPeak > smplCnt && smplCnt > 7)
          minPeak = smplCnt;
        peakcnt++;
        if (g_debugMode == 2)
          prnt("DEBUG NRZ: minPeak: %d, smplCnt: %d, peakcnt: %d", minPeak,
               smplCnt, peakcnt);
        smplCnt = 0;
      }
    }
  ...
  for (clkCnt = 0; clkCnt < ARRAYLEN(bestStart); ++clkCnt) {
    ...
    if (clk[clkCnt] < minPeak - (clk[clkCnt] / 4))
      continue;
    ...
    for (ii = 20; ii < loopCnt; ++ii) {
      if ((dest[ii] >= peak) || (dest[ii] <= low)) {
        ...
        for (i = ii; i < size - 20; ++i) {
          ...
          if ((i >= lastBit + clk[clkCnt] - tol) &&
              (i <= lastBit + clk[clkCnt] + tol)) {
            // test high/low
            if (dest[i] >= peak || dest[i] <= low) {
              // if same peak don't count it
              if ((dest[i] >= peak && !lastPeakHigh) ||
                  (dest[i] <= low && lastPeakHigh)) {
                peakcnt++;
              }
  ...
  for (int m = ARRAYLEN(peaksdet) - 1; m >= 0; m--) {
    if ((peaksdet[m] >= (peaksdet[best] - 1)) &&
        (peaksdet[m] <= peaksdet[best] + 1) && lowestTransition) {
      if (clk[m] > (lowestTransition - (clk[m] / 8)) &&
          clk[m] < (lowestTransition + (clk[m] / 8))) {
        best = m;
      }
    ...
    if (g_debugMode == 2)
      prnt("DEBUG NRZ: Clk: %d, peaks: %d, minPeak: %d, bestClk: %d, "
           "lowestTrs: %d",
           clk[m], peaksdet[m], minPeak, clk[best], lowestTransition);
  }
...
uint16_t countFC(const uint8_t *bits, size_t size, bool fskAdj) {
  ...
  for (; i < size - 20; i++) {
    if (bits[i] > bits[i - 1] && bits[i] >= bits[i + 1]) {
      ...
      fcCounter++;
      if (fskAdj) {
        // if we had 5 and now have 9 then go back to 8 (for when we get a fc 9
        // instead of an 8)
        if (lastFCcnt == 5 && fcCounter == 9)
          fcCounter--;

        // if fc=9 or 4 add one (for when we get a fc 9 instead of 10 or a 4
        // instead of a 5)
        if ((fcCounter == 9) || fcCounter == 4)
          fcCounter++;
  ...
  for (i = 0; i < 15; i++) {
    ...
    if (g_debugMode == 2)
      prnt("DEBUG countfc: FC %u, Cnt %u, best fc: %u, best2 fc: %u", fcLens[i],
           fcCnts[i], fcLens[best1], fcLens[best2]);
    if (fcLens[i] == 0)
      break;
  }
  ...
  if ((size - 180) / fcH / 3 > fcCnts[best1] + fcCnts[best2]) {
      if (g_debugMode == 2) prnt("DEBUG countfc: fc is too large: %zu > %u. Not
  psk or fsk", (size - 180) / fcH / 3, fcCnts[best1] + fcCnts[best2]); return 0;
  //lots of waves not psk or fsk
  }
  */
...
int DetectPSKClock(uint8_t *dest, size_t size, int clock,
                   size_t *firstPhaseShift, uint8_t *curPhase, uint8_t *fc) {
  uint16_t clk[] = {255, 16,  32,  40,  50, 64,
                    100, 128, 256, 272, 384}; // 255 is not a valid clock
  ...
  *fc = fcs & 0xFF;
  ...
  if (g_debugMode == 2)
    prnt("DEBUG PSK: FC: %d, FC2: %d", *fc, fcs >> 8);
  ...
  uint16_t bestErr[] = {1000, 1000, 1000, 1000, 1000, 1000,
                        1000, 1000, 1000, 1000, 1000};
  ...
  firstFullWave =
      pskFindFirstPhaseShift(dest, size, curPhase, i, *fc, &fullWaveLen);
  if (firstFullWave == 0) {
    // no phase shift detected - could be all 1's or 0's - doesn't matter where
    // we start so skip a little to ensure we are past any Start Signal
    firstFullWave = 160;
    fullWaveLen = 0;
  }
  ...
  *firstPhaseShift = firstFullWave;
  ...
  for (clkCnt = 9; clkCnt >= 1; clkCnt--) {
    ...
    uint16_t peakcnt = 0;
    if (g_debugMode == 2)
      prnt("DEBUG PSK: clk: %d, lastClkBit: %zu", clk[clkCnt], lastClkBit);
      ...
      if (dest[i] < dest[i + 1] && dest[i + 1] >= dest[i + 2]) {
        if (waveStart == 0) {
          ...
          if (waveLenCnt > *fc) {
            ...
            if (g_debugMode == 2)
              prnt("DEBUG PSK: phase shift at: %zu, len: %d, nextClk: %zu, i: "
                   "%zu, fc: %d",
                   waveStart, waveLenCnt, lastClkBit + clk[clkCnt] - tol, i + 1,
                   *fc);
            if (i + 1 >=
                lastClkBit + clk[clkCnt] - tol) { // should be a clock bit
              peakcnt++;
  ...
  for (i = 9; i >= 1; i--) {
    ...
    if (g_debugMode == 2)
      prnt("DEBUG PSK: Clk: %d, peaks: %d, errs: %d, bestClk: %d", clk[i],
           peaksdet[i], bestErr[i], clk[best]);
  }
  return clk[best];
}

// detects the bit clock for FSK given the high and low Field Clocks
uint8_t detectFSKClk(const uint8_t *bits, size_t size, uint8_t fcHigh,
                     uint8_t fcLow, int *firstClockEdge) {
  ...
  size_t i;
  uint8_t fcTol = ((fcHigh * 100 - fcLow * 100) / 2 + 50) /
                  100; //(uint8_t)(0.5+(float)(fcHigh-fcLow)/2);
  ...
  for (i = 0; i < 15; i++) {
    // get highest 2 RF values  (might need to get more values to compare or
    // compare all?)
  ...
  //   we could have mistakenly made a 9 a 10 instead of an 8 or visa versa so
  //   rfLens could be 1 FC off
  uint8_t tol1 = fcHigh + 1;

  if (g_debugMode == 2)
    prnt("DEBUG FSK: most counted rf values: 1 %d, 2 %d, 3 %d",
         rfLens[rfHighest], rfLens[rfHighest2], rfLens[rfHighest3]);

  // loop to find the highest clock that has a remainder less than the tolerance
  //   compare samples counted divided by
  // test 128 down to 32 (shouldn't be possible to have fc/10 & fc/8 and rf/16
  // or less)
  int m = 7;
  for (; m >= 2; m--) {
    if (rfLens[rfHighest] % clk[m] < tol1 ||
        rfLens[rfHighest] % clk[m] > clk[m] - tol1) {
      if (rfLens[rfHighest2] % clk[m] < tol1 ||
          rfLens[rfHighest2] % clk[m] > clk[m] - tol1) {
        if (rfLens[rfHighest3] % clk[m] < tol1 ||
            rfLens[rfHighest3] % clk[m] > clk[m] - tol1) {
          if (g_debugMode == 2)
            prnt("DEBUG FSK: clk %d divides into the 3 most rf values within "
                 "tolerance",
                 clk[m]);
          break;
        }
  ...
  if (m < 2)
    return 0; // oops we went too far

  return clk[m];
}

// **********************************************************************************************
// --------------------Modulation Demods &/or Decoding
// Section-----------------------------------
// **********************************************************************************************

// look for Sequence Terminator - should be pulses of clk*(1 or 2), clk*2,
// clk*(1.5 or 2), by idx we mean graph position index...
static bool findST(int *stStopLoc, int *stStartIdx, const int lowToLowWaveLen[],
                   const int highToLowWaveLen[], int clk, int tol, int buffSize,
                   size_t *i) {
  if (buffSize < *i + 4)
    return false;
  ...
  for (; *i < buffSize - 4; *i += 1) {
    *stStartIdx +=
        lowToLowWaveLen[*i]; // caution part of this wave may be data and part
                             // may be ST....  to be accounted for in main
                             // function for now...
    if (lowToLowWaveLen[*i] >= clk * 1 - tol &&
        lowToLowWaveLen[*i] <= (clk * 2) + tol &&
        highToLowWaveLen[*i] <
            clk + tol) { // 1 to 2 clocks depending on 2 bits prior
      if (lowToLowWaveLen[*i + 1] >= clk * 2 - tol &&
          lowToLowWaveLen[*i + 1] <= clk * 2 + tol &&
          highToLowWaveLen[*i + 1] >
              clk * 3 / 2 - tol) { // 2 clocks and wave size is 1 1/2
        if (lowToLowWaveLen[*i + 2] >= (clk * 3) / 2 - tol &&
            lowToLowWaveLen[*i + 2] <= clk * 2 + tol &&
            highToLowWaveLen[*i + 2] >
                clk -
                    tol) { // 1 1/2 to 2 clocks and at least one full clock wave
          if (lowToLowWaveLen[*i + 3] >= clk * 1 - tol &&
              lowToLowWaveLen[*i + 3] <=
                  clk * 2 + tol) { // 1 to 2 clocks for end of ST + first bit
            *stStopLoc = *i + 3;
            return true;
          }
        }
...
bool DetectST(uint8_t *buffer, size_t *size, int *foundclock, size_t *ststart,
              size_t *stend) {
  size_t bufsize = *size;
  // need to loop through all samples and identify our clock, look for the ST
  // pattern
  int clk = 0;
  int tol = 0;
  int j = 0, high, low, skip = 0, start = 0, end = 0, minClk = 255;
  size_t i = 0;
  // probably should calloc... || test if memory is available ... handle device
  // side? memory danger!!! [marshmellow]
  int tmpbuff[bufsize /
              LOWEST_DEFAULT_CLOCK]; // low to low wave count //guess rf/32
                                     // clock, if click is smaller we will only
                                     // have room for a fraction of the samples
                                     // captured
  int waveLen[bufsize /
              LOWEST_DEFAULT_CLOCK]; // high to low wave count //if clock is
                                     // larger then we waste memory in array
                                     // size that is not needed...
  ...
  if (!loadWaveCounters(buffer, bufsize, tmpbuff, waveLen, &j, &skip, &minClk,
                        &high, &low))
    return false;
  ...
  if (!clk) {
    if (g_debugMode == 2)
      prnt("DEBUG STT: clock not found - quitting");
    return false;
  }
  *foundclock = clk;
  ...
  if (!findST(&start, &skip, tmpbuff, waveLen, clk, tol, j, &i)) {
    ...
    if (g_debugMode == 2)
      prnt("DEBUG STT: first STT not found - quitting");
    return false;
  } else {
    if (g_debugMode == 2)
      prnt("DEBUG STT: first STT found at wave: %i, skip: %i, j=%i", start,
           skip, j);
  }
  ...
  skip += clk * 7 / 2; // 3.5 clocks from tmpbuff[i] = end of st - also aligns
                       // for ending point
  ...
  if (!findST(&dummy1, &end, tmpbuff, waveLen, clk, tol, j, &i)) {
    ...
    if (g_debugMode == 2)
      prnt("DEBUG STT: second STT not found - quitting");
    return false;
  }
  end -= phaseoff;
  if (g_debugMode == 2)
    prnt("DEBUG STT: start of data: %d end of data: %d, datalen: %d, clk: %d, "
         "bits: %d, phaseoff: %d",
         skip, end, end - skip, clk, (end - skip) / clk, phaseoff);
  // now begin to trim out ST so we can use normal demod cmds
  start = skip;
  size_t datalen = end - start;
  // check validity of datalen (should be even clock increments)  - use a
  // tolerance of up to 1/8th a clock
  if (clk - (datalen % clk) <= clk / 8) {
    ...
    if (g_debugMode == 2)
      prnt(
          "DEBUG STT: datalen not divisible by clk: %zu %% %d = %zu - quitting",
          datalen, clk, datalen % clk);
    return false;
  }
  ...
  if (datalen / clk < 8 * 4) {
    if (g_debugMode == 2)
      prnt("DEBUG STT: datalen is less than 1 full t55xx block - quitting");
    return false;
  }
  size_t dataloc = start;
  if (buffer[dataloc - (clk * 4) - (clk / 4)] <= low &&
      buffer[dataloc] <= low && buffer[dataloc - (clk * 4)] >= high) {
    // we have low drift (and a low just before the ST and a low just after the
    // ST) - compensate by backing up the start
    for (i = 0; i <= (clk / 4); ++i) {
  ...
  i = 0;
  if (g_debugMode == 2)
    prnt("DEBUG STT: Starting STT trim - start: %zu, datalen: %zu ", dataloc,
         datalen);
  bool firstrun = true;
  // warning - overwriting buffer given with raw wave data with ST removed...
  while (dataloc < bufsize - (clk / 2)) {
    // compensate for long high at end of ST not being high due to signal
    // loss... (and we cut out the start of wave high part)
    if (buffer[dataloc] < high && buffer[dataloc] > low &&
        buffer[dataloc + clk / 4] < high && buffer[dataloc + clk / 4] > low) {
      for (i = 0; i < clk / 2 - tol; ++i) {
        buffer[dataloc + i] = high + 5;
      }
    } // test for small spike outlier (high between two lows) in the case of
      // very strong waves
    ...
    if (g_debugMode == 2)
      prnt("DEBUG STT: skipping STT at %zu to %zu", dataloc,
           dataloc + (clk * 4));
    dataloc += clk * 4;
  }
  *size = newloc;
...
// check for phase errors - should never have half a 1 or 0 by itself and should
// never exceed 1111 or 0000 in a row decodes miller encoded binary NOTE
// askrawdemod will NOT demod miller encoded ask unless the clock is manually
// set to 1/2 what it is detected as!
/*
static int millerRawDecode(uint8_t *bits, size_t *size, int invert) {
    ...
    // for now error if alignment not found.  later add option to run it with
multiple offsets... if (alignCnt != 4) { if (g_debugMode) prnt("ERROR
MillerDecode: alignment not found so either your bits is not miller or your data
does not have a 101 in it"); return -1;
    }
    alignedIdx = (i - 1) % 2;
    for (i = alignedIdx; i < *size - 3; i += 2) {
        halfClkErr = (uint8_t)((halfClkErr << 1 | bits[i]) & 0xFF);
        if ((halfClkErr & 0x7) == 5 || (halfClkErr & 0x7) == 2 || (i > 2 &&
(halfClkErr & 0x7) == 0) || (halfClkErr & 0x1F) == 0x1F) { errCnt++;
            bits[bitCnt++] = 7;
            continue;
        }
...
// check for phase errors - should never have 111 or 000 should be 01001011 or
// 10110100 for 1010 decodes biphase or if inverted it is AKA conditional
// dephase encoding AKA differential manchester encoding
...
uint16_t manrawdecode(uint8_t *bits, size_t *size, uint8_t invert,
                      uint8_t *alignPos) {
  ...
  for (uint8_t k = 0; k < 2; k++) {
    ...
    if (bestErr > errCnt) {
      ...
      bestRun = k;
      if (g_debugMode == 2)
        prnt("DEBUG manrawdecode: bestErr %d | bestRun %u", bestErr, bestRun);
    }
    errCnt = 0;
  }
...
static uint16_t cleanAskRawDemod(uint8_t *bits, size_t *size, int clk,
                                 int invert, int high, int low, int *startIdx) {
  *startIdx = 0;
      ...
      if ((bits[i] >= high && !waveHigh) || (bits[i] <= low && waveHigh)) {
        ...
        if (smplCnt > clk - cl_4 - 1) { // full clock
          ...
          if (smplCnt > clk + cl_4 + 1) {
            ...
            errCnt++;
            if (g_debugMode == 2)
              prnt("DEBUG ASK: cleanAskRawDemod ASK Modulation Error FULL at: "
                   "%zu  [%zu > %u]",
                   i, smplCnt, clk + cl_4 + 1);
            bits[bitCnt++] = 7;
          } else if (waveHigh) {
            bits[bitCnt++] = invert;
            bits[bitCnt++] = invert;
          ...
          if (smplCnt > cl_2 + cl_4 + 1) { // too many samples
            errCnt++;
            if (g_debugMode == 2)
              prnt("DEBUG ASK: cleanAskRawDemod ASK Modulation Error HALF at: "
                   "%zu  [%zu]",
                   i, smplCnt);
            bits[bitCnt++] = 7;
          }
          ...
          if (*startIdx == 0) {
            *startIdx = i - cl_2;
            if (g_debugMode == 2)
              prnt("DEBUG ASK: cleanAskRawDemod minus half clock [%d]",
                   *startIdx);
          }
          waveHigh = !waveHigh;
          smplCnt = 0;
  ...
  *size = bitCnt;
  ...
  if (g_debugMode == 2)
    prnt("DEBUG ASK: cleanAskRawDemod Startidx %d", *startIdx);

  return errCnt;
}

// attempts to demodulate ask modulations, askType == 0 for ask/raw, askType==1
// for ask/manchester
int askdemod_ext(uint8_t *bits, size_t *size, int *clk, int *invert, int maxErr,
                 uint8_t amp, uint8_t askType, int *startIdx) {
  ...
  if (signalprop.isnoise) {
    if (g_debugMode == 2)
      prnt("DEBUG (askdemod_ext) just noise detected - aborting");
    return -2;
  }
  ...
  if (g_debugMode == 2)
    prnt("DEBUG (askdemod_ext) clk %d, beststart %d, amp %d", *clk, start, amp);
  ...
  if (DetectCleanAskWave(bits, *size, high, low)) {
    ...
    *startIdx = start - (*clk / 2);
    if (g_debugMode == 2)
      prnt("DEBUG: (askdemod_ext) Clean wave detected  --- startindex %d",
           *startIdx);
    ...
    if (askType) { // ask/manchester
      ...
      *startIdx += ((*clk / 2) * alignPos);
      ...
      if (g_debugMode == 2)
        prnt("DEBUG: (askdemod_ext) CLEAN: startIdx %i, alignPos %u , "
             "bestError %zu",
             *startIdx, alignPos, errCnt);
    }
    return errCnt;
  }
  ...
  *startIdx = start - (*clk / 2);
  ...
  if (*clk <= 32)
    tol = 1; // clock tolerance may not be needed anymore currently set to + or
             // - 1 but could be increased for poor waves or removed entirely
  ...
  for (i = start; i < *size; ++i) {
    if (i - lastBit >= *clk - tol) {
      if (bits[i] >= high) {
        ...
        if (bitnum > 0) {
          //                    if (g_debugMode == 2) prnt("DEBUG:
          //                    (askdemod_ext) Modulation Error at: %u", i);
          bits[bitnum++] = 7;
          errCnt++;
        }
...
int askdemod(uint8_t *bits, size_t *size, int *clk, int *invert, int maxErr,
             uint8_t amp, uint8_t askType) {
  int start = 0;
  return askdemod_ext(bits, size, clk, invert, maxErr, amp, askType, &start);
}

// demodulate NRZ wave - requires a read with strong signal
// peaks invert bit (high=1 low=0) each clock cycle = 1 bit determined by last
// peak
int nrzRawDemod(uint8_t *dest, size_t *size, int *clk, const int *invert,
                int *startIdx) {

  if (signalprop.isnoise) {
    if (g_debugMode == 2)
      prnt("DEBUG nrzRawDemod: just noise detected - quitting");
    return -1;
  }
  ...
  *clk = DetectNRZClock(dest, *size, *clk, &clkStartIdx);
  ...
  // now demod based on clock (rf/32 = 32 1's for one 1 bit, 32 0's for one 0
  // bit)
  size_t lastBit = 0;
  size_t numBits = 0;
  for (i = 21; i < *size - 20; i++) {
    // if transition detected or large number of same bits - store the passed
    // bits
    if (dest[i] != dest[i - 1] || (i - lastBit) == (10 * *clk)) {
      memset(dest + numBits, dest[i - 1] ^ *invert,
             (i - lastBit + (*clk / 4)) / *clk);
      numBits += (i - lastBit + (*clk / 4)) / *clk;
      if (lastBit == 0) {
        *startIdx = i - (numBits * *clk);
        if (g_debugMode == 2)
          prnt("DEBUG NRZ: startIdx %i", *startIdx);
      }
      lastBit = i - 1;
    }
  }
...
// translate wave to 11111100000 (1 for each short wave [higher freq] 0 for each
// long wave [lower freq])
static size_t fsk_wave_demod(uint8_t *dest, size_t size, uint8_t fchigh,
                             uint8_t fclow, int *startIdx) {
  ...
  // Lets define some expected lengths. FSK1 is easier since it has bigger
  // differences between. FSK1 8/5 50/8 = 6         | 40/8 = 5  | 64/8 = 8 50/5
  // = 10        | 40/5 = 8  | 64/5 = 12
  ...
  // 50/8  = 6 (5-7)  | 40/8  = 5 (4-6)       | 64/8  = 8 (7-9)
  //
  // It easy to see to the overgaping, but luckily we the group value also, like
  // 1111000001111 to separate between which bit to demodulate to.
  ...
  // the 1-0 to 0-1  width should be divided with exp_zero.   Ie: 3+5+6+7 = 21/6
  // = 3
    ...
    if (dest[idx - 1] < dest[idx]) {
      ...
      if (currSample < (fclow - 2)) { // 0-5 = garbage noise (or 0-3)
        // do nothing with extra garbage
                 ...
                 (fchigh - 1)) { // 6-8 = 8 sample waves  (or 3-6 = 5)
        // correct previous 9 wave surrounded by 8 waves (or 6 surrounded by 5)
        if (numBits > 1 && LastSample > (fchigh - 2) &&
            (preLastSample < (fchigh - 1))) {
          dest[numBits - 1] = 1;
        }
        dest[numBits++] = 1;
      ...
      } else if (currSample > (fchigh + 1) &&
                 numBits < 3) { // 12 + and first two bit = unusable garbage
        // do nothing with beginning garbage and reset..  should be rare..
        numBits = 0;
      } else if (currSample == (fclow + 1) &&
                 LastSample ==
                     (fclow - 1)) { // had a 7 then a 9 should be two 8's (or 4
                                    // then a 6 should be two 5's)
        dest[numBits++] = 1;
        if (numBits > 0 && *startIdx == 0) {
          *startIdx = idx - fclow;
        }
  ...
  return numBits; // Actually, it returns the number of bytes, but each byte
                  // represents a bit: 1 or 0
}

// translate 11111100000 to 10
// rfLen = clock, fchigh = larger field clock, fclow = smaller field clock
static size_t aggregate_bits(uint8_t *dest, size_t size, uint8_t clk,
    ...
    if (numBits == 0) {
      if (lastval == 1) { // high to low
        *startIdx += (fclow * i) - (n * clk);
        if (g_debugMode == 2)
          prnt(
              "DEBUG (aggregate_bits) FSK startIdx %i, fclow*idx %zu, n*clk %u",
              *startIdx, fclow * i, n * clk);
      } else {
        *startIdx += (fchigh * i) - (n * clk);
        if (g_debugMode == 2)
          prnt("DEBUG (aggregate_bits) FSK startIdx %i, fchigh*idx %zu, n*clk "
               "%u",
               *startIdx, fchigh * i, n * clk);
      }
    }
  ...
  if (n > clk / fchigh) {
    ...
    numBits += n;
    if (g_debugMode == 2)
      prnt("DEBUG (aggregate_bits) extra bits in the end");
  }
  return numBits;
}

// full fsk demod from GraphBuffer wave to decoded 1s and 0s (no mandemod)
size_t fskdemod(uint8_t *dest, size_t size, uint8_t rfLen, uint8_t invert,
                uint8_t fchigh, uint8_t fclow, int *start_idx) {
  if (signalprop.isnoise)
    return 0;
  // FSK demodulator
  size = fsk_wave_demod(dest, size, fchigh, fclow, start_idx);
  if (g_debugMode == 2)
    prnt("DEBUG (fskdemod) got %zu bits", size);
...
// TODO: Iceman - hard coded value 7,  should be #define
int pskRawDemod_ext(uint8_t *dest, size_t *size, int *clock, const int *invert,
                    int *startIdx) {
  ...
  uint8_t fc = 0;
  size_t i = 0, numBits = 0, waveStart = 1, waveEnd, firstFullWave = 0,
         lastClkBit = 0;
  ...
  *clock = DetectPSKClock(dest, *size, *clock, &firstFullWave, &curPhase, &fc);
  ...
  if (firstFullWave == 0) {
    ...
    firstFullWave =
        pskFindFirstPhaseShift(dest, *size, &curPhase, i, fc, &fullWaveLen);
    if (firstFullWave == 0) {
      // no phase shift detected - could be all 1's or 0's - doesn't matter
      // where we start so skip a little to ensure we are past any Start Signal
      firstFullWave = 160;
  ...
  *startIdx = firstFullWave - (*clock * numBits) + 2;
  ...
  if (g_debugMode == 2) {
    prnt("DEBUG PSK: firstFullWave: %zu, waveLen: %u, startIdx %i",
         firstFullWave, fullWaveLen, *startIdx);
    ...
    if (dest[i] + fc < dest[i + 1] && dest[i + 1] >= dest[i + 2]) {
      if (waveStart == 0) {
        ...
        if (waveLenCnt > fc) {
          ...
          dest[numBits++] = curPhase;
        } else if (waveLenCnt < fc - 1) { // wave is smaller than field clock
...
int Em410xDecode(uint8_t *bits, size_t *size, size_t *start_idx, uint32_t *hi,
                 uint64_t *lo) {
  // sanity check
  if (bits[1] > 1)
    return -1;
  ...
  *size = removeEm410xParity(bits, *start_idx + sizeof(preamble), *size == 128,
                             &validShort, &validShortExtended, &validLong);

  if (validShort) {
    // std em410x format
    *hi = 0;
    *lo = ((uint64_t)(bytebits_to_byte(bits, 8)) << 32) |
          (bytebits_to_byte(bits + 8, 32));
    // 1 = Short
    return 1;
  }
  if (validShortExtended || validLong) {
    ...
    *lo = ((uint64_t)(bytebits_to_byte(bits + 24, 32)) << 32) |
          (bytebits_to_byte(bits + 24 + 32, 32));
    // 2 = Long
    // 4 = ShortExtended
...
int HIDdemodFSK(uint8_t *dest, size_t *size, uint32_t *hi2, uint32_t *hi,
                uint32_t *lo, int *waveStartIdx) {
  ...
  *size = fskdemod(dest, *size, 50, 1, 10, 8, waveStartIdx); // hid fsk2a
  ...
  // 00011101 bit pattern represent start of frame, 01 pattern represents a 0
  // and 10 represents a 1
  size_t start_idx = 0;
  ...
  // final loop, go over previously decoded FSK data and manchester decode into
  // usable tag ID
  for (size_t idx = num_start; (idx - num_start) < *size - sizeof(preamble);
       idx += 2) {
...
int detectIOProx(uint8_t *dest, size_t *size, int *waveStartIdx) {
  ...
  *size = fskdemod(dest, *size, 64, 1, 10, 8, waveStartIdx); // io fsk2a
  ...
  // 0           10          20          30          40          50          60
  //|           |           |           |           |           |           |
  // 01234567 8 90123456 7 89012345 6 78901234 5 67890123 4 56789012 3 45678901
  // 23
  //-----------------------------------------------------------------------------
  // 00000000 0 11110000 1 facility 1 version* 1 code*one 1 code*two 1 ????????
  // 11
  //
  ...
  if (!dest[start_idx + 8] && dest[start_idx + 17] == 1 &&
      dest[start_idx + 26] == 1 && dest[start_idx + 35] == 1 &&
      dest[start_idx + 44] == 1 && dest[start_idx + 53] == 1) {
