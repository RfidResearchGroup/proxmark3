 ...
 * append trace data from RAM to a file in flash (hf_14asniff.trace) and
 * unmount.
 *
...
static void DownloadTraceInstructions(void) {
  ...
  Dbprintf("1. mem spiffs dump -s " HF_14ASNIFF_LOGFILE
           " -d hf_14asniff.trace");
...
void RunMod(void) {
  ...
  if (trace_len > 0) {
    ...
    if (!exists_in_spiffs(HF_14ASNIFF_LOGFILE)) {
      rdv40_spiffs_write(HF_14ASNIFF_LOGFILE, trace_buffer, trace_len,
                         RDV40_SPIFFS_SAFETY_SAFE);
      Dbprintf("[!] Wrote trace to " HF_14ASNIFF_LOGFILE);
    } else {
      rdv40_spiffs_append(HF_14ASNIFF_LOGFILE, trace_buffer, trace_len,
                          RDV40_SPIFFS_SAFETY_SAFE);
