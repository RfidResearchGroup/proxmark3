...
uint16_t Flash_ReadData(uint32_t address, uint8_t *out, uint16_t len) {
  ...
  if (!len || Flash_CheckBusy(BUSY_TIMEOUT))
    return 0;
...
/* This ensures we can ReadData without having to cycle through initialization
 * every time */
...
uint16_t Flash_WriteData(uint32_t address, uint8_t *in, uint16_t len) {
  ...
  if (((address & 0xFF) + len) > 256) {
    Dbprintf("Flash_WriteData 256 fail [ 0x%02x ] [ %u ]",
             (address & 0xFF) + len, len);
    return 0;
  }
...
uint16_t Flash_WriteDataCont(uint32_t address, uint8_t *in, uint16_t len) {
  ...
  if (((address & 0xFF) + len) > 256) {
    Dbprintf("Flash_WriteDataCont 256 fail [ 0x%02x ] [ %u ]",
             (address & 0xFF) + len, len);
    return 0;
  }
...
bool Flash_Erase4k(uint8_t block, uint8_t sector) {
  ...
  if (block > MAX_BLOCKS || sector > MAX_SECTORS)
    return false;
...
bool Flash_Erase32k(uint32_t address) {
    if (address & (32*1024 - 1)) {
        if ( g_dbglevel > 1 ) Dbprintf("Flash_Erase32k : Address is not align at
4096"); return false;
    }
...
void Flashmem_print_status(void) {
  ...
  Dbprintf("  Baudrate................ " _GREEN_("%d MHz"),
           FLASHMEM_SPIBAUDRATE / 1000000);
  ...
  Flash_UniqueID(uid);
  Dbprintf("  Unique ID (be).......... " _YELLOW_(
               "0x%02X%02X%02X%02X%02X%02X%02X%02X"),
           uid[0], uid[1], uid[2], uid[3], uid[4], uid[5], uid[6], uid[7]);
  if (g_dbglevel > 3) {
    Dbprintf("  Unique ID (le).......... " _YELLOW_(
                 "0x%02X%02X%02X%02X%02X%02X%02X%02X"),
...
void Flashmem_print_info(void) {
  ...
  if (isok == 2) {
    ...
    if (num != 0xFFFF && num != 0x0)
      Dbprintf("  Mifare.................. "_YELLOW_("%u") " / "_GREEN_(
                   "%u") " keys",
               num, DEFAULT_MF_KEYS_MAX);
  }
  ...
  if (isok == 2) {
    num = ((keysum[1] << 8) | keysum[0]);
    if (num != 0xFFFF && num != 0x0)
      Dbprintf("  T55x7................... "_YELLOW_("%u") " / "_GREEN_(
                   "%u") " keys",
               num, DEFAULT_T55XX_KEYS_MAX);
  }
  ...
  if (isok == 2) {
    num = ((keysum[1] << 8) | keysum[0]);
    if (num != 0xFFFF && num != 0x0)
      Dbprintf("  iClass.................. "_YELLOW_("%u") " / "_GREEN_(
                   "%u") " keys",
               num, DEFAULT_ICLASS_KEYS_MAX);
  }
...
void FlashSetup(uint32_t baudrate) {
  ...
  AT91C_BASE_PIOA->PIO_PDR |=
      (GPIO_NCS0 | GPIO_MISO | GPIO_MOSI | GPIO_SPCK | GPIO_NCS2);
  ...
  AT91C_BASE_SPI->SPI_MR =
      ...
      SPI_PCS(
          SPI_CSR_NUM) |   // Peripheral Chip Select (selects SPI_NCS2 or PA10)
  ...
  AT91C_BASE_SPI->SPI_CSR[2] =
      SPI_DLYBCT(dlybct,
                 MCK) | // Delay between Consecutive Transfers (32 MCK periods)
      ...
      //  If the SPI is programmed with CSAAT = 1, SCBR(baudrate) = 1 and two
      //  transfers are performed consecutively on the same slave with an IDLE
      //  state between them, the tx_ready signal does not rise after the second
      //  data has been transferred in the shifter. This can imply for example,
      //  that the second data is sent twice. COLIN :: For now we STILL use
      //  CSAAT=1 to avoid having to (de)assert  NPCS manually via PIO lines and
      //  we deal with delay
      (csaat << 3) |
      /* Spi modes:
          ...
          Update: for 24MHz, writing is more stable with ncpha=1, else bitflips
         occur.
      */
      (ncpha << 1) | // Clock Phase data captured on leading edge, changes on
                     // following edge
...
uint16_t FlashSendByte(uint32_t data) {
  ...
  // if you are checking for incoming data returned then the TXEMPTY flag is
  // redundant while ((AT91C_BASE_SPI->SPI_SR & AT91C_SPI_TXEMPTY) == 0) {};
