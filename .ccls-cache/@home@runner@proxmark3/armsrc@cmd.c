...
// "Session" flag, to tell via which interface next msgs should be sent: USB or
// FPC USART
bool g_reply_via_fpc = false;
bool g_reply_via_usb = false;

int reply_old(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
...
#ifdef WITH_FPC_USART_HOST
    resultfpc =
        usart_writebuffer_sync((uint8_t *)&txcmd, sizeof(PacketResponseOLD));
#else
...
static int reply_ng_internal(uint16_t cmd, int16_t status, const uint8_t *data,
                             size_t len, bool ng) {
  ...
  // require CRC
  if ((g_reply_via_fpc && g_reply_with_crc_on_fpc) ||
      ((g_reply_via_usb) && g_reply_with_crc_on_usb)) {
...
int reply_mix(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
              const void *data, size_t len) {
  ...
  int res = reply_ng_internal((cmd & 0xFFFF), status, cmddata,
                              len + sizeof(arg), false);
  return res;
}

static int receive_ng_internal(PacketCommandNG *rx,
                               uint32_t read_ng(uint8_t *data, size_t len),
                               bool usb, bool fpc) {
  PacketCommandNGRaw rx_raw;
  size_t bytes =
      read_ng((uint8_t *)&rx_raw.pre, sizeof(PacketCommandNGPreamble));
  ...
  if (rx->magic == COMMANDNG_PREAMBLE_MAGIC) { // New style NG command
    ...
    // Get the postamble
    bytes =
        read_ng((uint8_t *)&rx_raw.foopost, sizeof(PacketCommandNGPostamble));
    ...
    if (rx->crc != COMMANDNG_POSTAMBLE_MAGIC) {
      ...
      compute_crc(CRC_14443_A, (uint8_t *)&rx_raw,
                  sizeof(PacketCommandNGPreamble) + length, &first, &second);
    ...
    bytes = read_ng(((uint8_t *)&rx_old) + sizeof(PacketCommandNGPreamble),
                    sizeof(PacketCommandOLD) - sizeof(PacketCommandNGPreamble));
