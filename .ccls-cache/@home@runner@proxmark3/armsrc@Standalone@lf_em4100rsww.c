...
//-----------------------------------------------------------------------------
// LF rsww   -  This mode can read EM4100 tag, save it to flash (RDV4 only),
// emulate it, clone it to T55xx tag, validate the write and wipe T55xx tag.
//
...
//
//              To recall only LAST stored ID from flash use lf-last instead of
//              lf file.
//
//-----------------------------------------------------------------------------
// Modes of operation:
//
// --- Read ---
// Proxmark reads an EM4100 tag. LED A is turned on. When the tag is detected,
// it is saved to flash (RDV4 only) and proxmark enters the emulation mode. It's
// the default mode for non-RDV4 devices, and if no previous read is present in
// the flash it's the default mode for RDV4 devices. Pressing the button exists
// reading mode and enters emulation mode (only if any read is present in the
// memory). Double pressing the button enters wiping mode.
//
// --- Emulate ---
...
// When writing is complete LEDs A and B blink three times and proxmark enters
// the emulation mode.
//
// --- Validate ---
// Proxmark reads an EM4100 tag. LED C is turned on.
// If tag matches the last saved tag, LED C blinks three times. If it doesn't
// all LEDs blink three times. Proxmark enters the emulation mode afterwards.
// The result of the read is DISCARDED.
// Pressing the button enters the emulation mode.
//
// --- Wipe ---
// Proxmark continously wipes all approached T55xx tags. LED D is turned on,
// LEDs A-C are blinking. Pressing the button enters the default mode (reading
// or emulation).
//-----------------------------------------------------------------------------
...
static bool ReadFlash(void) {
  ...
  uint8_t bt[5];
  if (rdv40_spiffs_read(filenameLast, (uint8_t *)&bt, 5,
                        RDV40_SPIFFS_SAFETY_NORMAL) < 0)
    return false;
...
static void Wipe(void) {
  ...
  for (;;) {
    ...
    LED_C_ON();
    LED_D_ON();
    copy_em410x_to_t55xx(LF_RWSB_T55XX_TYPE, LF_CLOCK, (uint32_t)0, (uint32_t)0,
                         false, false);
    SpinDelay(60);
    LEDsoff();
    LED_D_ON();
...
static void Write(void) {
  ...
  LED_A_ON();
  LED_B_ON();
  copy_em410x_to_t55xx(LF_RWSB_T55XX_TYPE, LF_CLOCK, (uint32_t)(low >> 32),
                       (uint32_t)(low & 0xffffffff), false, false);
  SpinDelay(75);
  LEDsoff();
