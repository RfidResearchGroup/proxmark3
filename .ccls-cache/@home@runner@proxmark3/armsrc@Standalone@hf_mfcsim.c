 ...
 * Data is read from bin dump file uploaded to flash memory
 * (hf_mfcsim_dump_xx.bin). Only support mifare classic 1k
 *
 * To upload input file (eml format) to flash:
 * - mem spiffs upload -s <filename> -d hf_mfcsim_dump_xx.bin (Notes: xx is form
 * 01 to 15) To delete the input file from flash:
...
static bool fill_eml_from_file(char *dumpfile) {
  ...
  if (size != DUMP_SIZE) {
    Dbprintf(_RED_("File Size: %dB  The dump file size is incorrect! Only "
                   "support Mifare Classic 1K! Please check it."));
    BigBuf_free();
    return false;
  }
  ...
  rdv40_spiffs_read_as_filetype(dumpfile, emCARD, size,
...
void RunMod(void) {
  ...
  for (int i = 1;; i++) {
    ...
    Dbprintf(_YELLOW_("[Slot: %d] Try to load dump file: %s"), i,
             cur_dump_file);
    ...
    Dbprintf(
        _YELLOW_(
            "[Slot: %d] Simulation start, Press button to change next card."),
        i);
    uint16_t simflags = FLAG_UID_IN_EMUL | FLAG_MF_1K;
    Mifare1ksim(simflags, 0, NULL, 0, 0);
    Dbprintf(_YELLOW_("[Slot: %d] Simulation end, Write Back to dump file!"),
             i);

    // Simulation end, Write Back
    if (!write_file_from_eml(cur_dump_file)) {
      Dbprintf(_RED_("[Slot: %d] Write Failed! Anyway, Change to next one!"),
               i);
      continue;
    }
