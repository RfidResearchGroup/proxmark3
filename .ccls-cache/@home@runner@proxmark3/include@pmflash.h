...
// -1 for historical compatibility with already released Proxmark3 RDV4.0
// devices
#define FLASH_MEM_SIGNATURE_OFFSET                                             \
  (FLASH_MEM_MAX_SIZE - FLASH_MEM_SIGNATURE_LEN - 1)
#endif
...
#define DEFAULT_ICLASS_KEYS_OFFSET                                             \
  (DEFAULT_T55XX_KEYS_OFFSET - DEFAULT_ICLASS_KEYS_LEN)
...
#define DEFAULT_MF_KEYS_OFFSET                                                 \
  (DEFAULT_ICLASS_KEYS_OFFSET - DEFAULT_MF_KEYS_LEN)
#define DEFAULT_MF_KEYS_MAX ((DEFAULT_MF_KEYS_LEN - 2) / 6)
#endif

// RDV40,  validation structure to help identifying that client/firmware is
// talking with RDV40
typedef struct {
  uint8_t magic[4];
