...
#define PMC_PLL_COUNT_BEFORE_LOCK(x)                                           \
  (((x)&0x3F) << 8)                                  // 6bit register 0011 1111
...
#define BUTTON_PRESS()                                                         \
  !((AT91C_BASE_PIOA->PIO_PDSR & GPIO_BUTTON) == GPIO_BUTTON)
#define WAIT_BUTTON_RELEASED()                                                 \
  {                                                                            \
    while (BUTTON_PRESS()) {                                                   \
      WDT_HIT();                                                               \
    };                                                                         \
...
#define USB_ATTACHED()                                                         \
  !((AT91C_BASE_PIOA->PIO_PDSR & GPIO_NVDD_ON) == GPIO_NVDD_ON)
...
typedef struct {
  int magic;    /* Magic sequence, to distinguish against random uninitialized
                   memory */
  char version; /* Must be 1 */
  char command;
  struct {
    unsigned int bootrom_present : 1; /* Set when a bootrom that is capable of
                                         parsing the common area is present */
    unsigned int osimage_present : 1; /* Set when a osimage that is capable of
                                         parsing the common area is present */
