...
// dest    char* buffer, where to put the UID, if NULL a static buffer is
// returned uid[]     the UID in transmission order return: ptr to string
char *iso15693_sprintUID(char *dest, uint8_t *uid) {
  ...
  if (uid) {
    ...
    sprintf(dest,
#endif
             "%02X %02X %02X %02X %02X %02X %02X %02X", uid[7], uid[6], uid[5],
