 ...
 *
 * The companion Android app is available on our gitlab:
 * gitlab.com/loudmouth-security
 *
...
void ModInfo(void) {
  DbpString("  HF - Long-range relay 14a over serial<->IP -  a.k.a. CardHopper "
            "(Sam Haskins)");
}
...
static const uint8_t magicEND[4] = "\xff"
                                   "END";
static const uint8_t magicRSRT[7] = "RESTART";
static const uint8_t magicERR[4] = "\xff"
                                   "ERR";
static uint8_t magicACK[1] =
    "\xfe"; // is constant, but must be passed to API that doesn't like that
...
static bool try_use_canned_response(const uint8_t *, int,
                                    tag_response_info_t *);
...
static bool GetIso14443aCommandFromReaderInterruptible(uint8_t *, uint8_t *,
                                                       int *);
...
static void become_card(void) {
  ...
  uint8_t pages;
  SimulateIso14443aInit(tagType, flags, data, &canned, &cuid, counters,
                        tearings, &pages);
  ...
  while (1) {
    ...
    if (!GetIso14443aCommandFromReaderInterruptible(fromReaderDat, parity,
                                                    &fromReaderLen)) {
      if (cardhopper_data_available()) {
        ...
        DbpString(
            _CYAN_("[@]") " Button pressed - Breaking from emulation loop");
        break;
      }
      continue;
    }
    ...
    if (try_use_canned_response(fromReaderDat, fromReaderLen, canned))
      continue;
...
static void prepare_emulation(uint8_t *tagType, uint16_t *flags, uint8_t *data,
                              packet_t *ats) {
  ...
  *flags = (uidRx.len == 10
                ...
                : (uidRx.len == 7 ? FLAG_7B_UID_IN_DATA : FLAG_4B_UID_IN_DATA));
  DbpString(_CYAN_("[@]") " UID:");
...
static void cook_ats(packet_t *ats, uint8_t fwi, uint8_t sfgi) {
  if (ats->len != ats->dat[0]) {
    DbpString(
        _RED_("[!]") " Malformed ATS - unable to cook; things may go wrong!");
    return;
  }

  uint8_t t0 = 0x70; // TA, TB, and TC transmitted, FSCI nibble set later
  uint8_t ta = 0x80; // only 106kbps rate supported, and must be same in both
                     // directions - PM3 doesn't support any other rates
  ...
  if (ats->len > 1) {
    ...
    if (orig_t0 & 0x10) {
      ...
      if (len < 1) {
        DbpString(_RED_(
            "[!]") " Malformed ATS - unable to cook; things may go wrong!");
        return;
      }
      orig_ats_ptr++;
      len--;
...
static bool try_use_canned_response(const uint8_t *dat, int len,
                                    tag_response_info_t *canned) {
  if ((dat[0] == ISO14443A_CMD_REQA || dat[0] == ISO14443A_CMD_WUPA) &&
      len == 1) {
  ...
  if ((dat[0] & 0xF7) == ISO14443A_CMD_WTX)
    return true; // bit 0x08 indicates CID following
  if (dat[0] == ISO14443A_CMD_HALT && len == 4)
    return true;
...
static void read_packet(packet_t *packet) {
  while (!cardhopper_data_available()) {
    ...
    if (BUTTON_PRESS()) {
      DbpString(
          _CYAN_("[@]") " Button pressed while waiting for packet - aborting");
      return;
    }
  }
  ...
  while (dataReceived != packet->len) {
    while (!cardhopper_data_available()) {
      ...
      if (BUTTON_PRESS()) {
        DbpString(
            _CYAN_("[@]") " Button pressed while reading packet - aborting");
        return;
      }
    }

    dataReceived +=
        cardhopper_read(packet->dat + dataReceived, packet->len - dataReceived);

    if (packet->len == 0x50 &&
        dataReceived >= sizeof(PacketResponseNGPreamble) &&
        packet->dat[0] == 0x4D && packet->dat[1] == 0x33 &&
        packet->dat[2] == 0x61) {
      // PM3 NG packet magic
...
static bool GetIso14443aCommandFromReaderInterruptible(uint8_t *received,
                                                       uint8_t *par, int *len) {
  LED_D_OFF();
