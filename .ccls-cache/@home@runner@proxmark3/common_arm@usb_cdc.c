...
#define USB_DESCRIPTOR_CONFIGURATION                                           \
  0x02 // DescriptorType for a Configuration Descriptor.
#define USB_DESCRIPTOR_STRING 0x03 // DescriptorType for a String Descriptor.
#define USB_DESCRIPTOR_INTERFACE                                               \
  0x04 // DescriptorType for an Interface Descriptor.
#define USB_DESCRIPTOR_ENDPOINT                                                \
  0x05 // DescriptorType for an Endpoint Descriptor.
#define USB_DESCRIPTOR_DEVICE_QUALIFIER                                        \
  0x06 // DescriptorType for a Device Qualifier.
#define USB_DESCRIPTOR_OTHER_SPEED                                             \
  0x07 // DescriptorType for a Other Speed Configuration.
#define USB_DESCRIPTOR_INTERFACE_POWER                                         \
  0x08                          // DescriptorType for Interface Power.
#define USB_DESCRIPTOR_OTG 0x09 // DescriptorType for an OTG Descriptor.
#define USB_DESCRIPTOR_IAD                                                     \
  0x0B // DescriptorType for a Interface Association Descriptor
...
static const char devDescriptor[] = {
    ...
    0x00,
    0x02, // Complies with USB Spec. Release (0200h = release 2.00)  0210 ==
          // release 2.10
    2,    // Device Class:    Communication Device Class
    0,    // Device Subclass: CDC class sub code ACM [ice 0x02 = win10 virtual
          // comport ]
    0,    // Device Protocol: CDC Device protocol (unused)
    AT91C_USB_EP_CONTROL_SIZE, // MaxPacketSize0
    0xc4,
    0x9a, // Vendor ID  [0x9ac4 = J. Westhues]
    0x8f,
    0x4b, // Product ID [0x4b8f = Proxmark-3 RFID Instrument]
    0x00,
    0x01, // BCD Device release number (1.00)
...
static const char cfgDescriptor[] = {
    ...
    */
    ...
    /* CDC Communication Class Interface Descriptor Requirement for
       Notification*/
    // -----------------------------------------------------------
    9,                        // Length
    ...
    2, // Capabilities        D1, Device supports the request combination of
    ...
    0, // Capabilities:       Device sends/receives call management information
       // only over the Communication Class interface. Device does not handle
...
// BOS descriptor
static const char bosDescriptor[] = {0x5,  USB_DESCRIPTOR_TYPE_BO,
                                     0xC,  0x0,
...
static const char CompatIDFeatureDescriptor[] = {
        0x28, 0x00, 0x00, 0x00,                         // Descriptor Length
40bytes (0x28) 0x00, 0x01,                                     // Version
('1.0') MS_EXTENDED_COMPAT_ID, 0x00,                    // Compatibility ID
Descriptor Index  0x0004 0x01,                                           //
Number of sections. 0x1 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,       //
Reserved (7bytes)
        // -----function section 1------
        0x00,                                           // Interface Number #0
        0x01,                                           // reserved (0x1)
        0x57, 0x49, 0x4E, 0x55, 0x53, 0x42, 0x00, 0x00, // Compatible ID
('WINUSB\0\0')  (8bytes) 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //
Sub-Compatible ID (8byte) 0x00, 0x00, 0x00, 0x00, 0x00, 0x00              //
Reserved (6bytes)
};
*/
...
static const char OSprop[] = {
        ...
        'A',0,'w',0,'e',0,'s',0,'o',0,'m',0,'e',0,' ',0,'P',0,'M',0,'3',0,'
...
static const char StrManufacturer[] = {
    ...
    'r',  0, 'k', 0, '.', 0, 'o', 0, 'r', 0, 'g', 0,
};

static const char StrProduct[] = {20,   // Length
                                  0x03, // Type is string
                                  'p',  0,   'r', 0,   'o', 0,   'x', 0,   'm',
                                  0,    'a', 0,   'r', 0,   'k', 0,   '3', 0};

#ifndef WITH_FLASH
static const char StrSerialNumber[] = {14,   // Length
                                       0x03, // Type is string
                                       'i',  0, 'c', 0, 'e', 0,
                                       'm',  0, 'a', 0, 'n', 0};
...
// offset 14, length  4: 2x unicode chars (underscores)      [[ to avoid
// descriptor being (size % 8) == 0, OS bug workaround ]] offset 18, length 32:
// 16x unicode chars (8-byte serial as hex characters)
// ============================
// total: 50 bytes
#define USB_STRING_DESCRIPTOR_SERIAL_NUMBER_LENGTH 50
char StrSerialNumber[] = {
    14,   // Length is initially identical to non-unique version ... The length
          // updated at boot, if unique serial is available
    0x03, // Type is string
    'i',  0, 'c', 0, 'e', 0, 'm', 0, 'a', 0, 'n', 0, '_', 0, '_', 0,
...
void usb_update_serial(uint64_t newSerialNumber) {
  ...
  if ((newSerialNumber == 0x0000000000000000) ||
      (newSerialNumber == 0xFFFFFFFFFFFFFFFF)) {
    return;
  }
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    uint8_t nibble1 =
        (newSerialNumber >> ((8 * i) + 4)) &
        0xFu; // bitmasks [0xF0, 0xF000, 0xF00000, ... 0xF000000000000000]
    uint8_t nibble2 =
        (newSerialNumber >> ((8 * i) + 0)) &
        0xFu; // bitmasks [0x0F, 0x0F00, 0x0F0000, ... 0x0F00000000000000]
...
// Bitmap for all status bits in CSR which must be written as 1 to cause no
// effect
...
void SetUSBreconnect(int value) { usb_reconnect = value; }
int GetUSBreconnect(void) { return usb_reconnect; }
void SetUSBconfigured(int value) { usb_configured = value; }
int GetUSBconfigured(void) { return usb_configured; }
...
uint32_t usb_read_ng(uint8_t *data, size_t len) {
  ...
  while (len) {
    ...
    if ((pUdp->UDP_CSR[AT91C_EP_OUT] & bank)) {
      ...
      uint32_t available =
          ((pUdp->UDP_CSR[AT91C_EP_OUT] & AT91C_UDP_RXBYTECNT) >> 16);
...
void AT91F_USB_SendZlp(AT91PS_UDP pudp) {
  ...
  // the flag for FIFO released is AT91C_UDP_TXCOMP rather than
  // AT91C_UDP_TXPKTRDY
  while (!(pudp->UDP_CSR[AT91C_EP_CONTROL] & AT91C_UDP_TXCOMP)) {
  };
  UDP_CLEAR_EP_FLAGS(AT91C_EP_CONTROL, AT91C_UDP_TXCOMP);
...
void AT91F_USB_SendStall(AT91PS_UDP pudp) {
  ...
  UDP_CLEAR_EP_FLAGS(AT91C_EP_CONTROL,
                     (AT91C_UDP_FORCESTALL | AT91C_UDP_ISOERROR));
  while (pudp->UDP_CSR[AT91C_EP_CONTROL] &
         (AT91C_UDP_FORCESTALL | AT91C_UDP_ISOERROR)) {
  };
}
  ...
  if ( bRequest == MS_VENDOR_CODE) {
      if ( bmRequestType == MS_WCID_GET_DESCRIPTOR ) { // C0
          if ( wIndex == MS_EXTENDED_COMPAT_ID ) {  // 4
              //AT91F_USB_SendData(pUdp, CompatIDFeatureDescriptor,
  MIN(sizeof(CompatIDFeatureDescriptor), wLength));
              //return;
          }
      }

      if ( bmRequestType == MS_WCID_GET_FEATURE_DESCRIPTOR ) {  //C1
          // if ( wIndex == MS_EXTENDED_PROPERTIES ) { // 5  - winusb bug with
  wIndex == interface index,  so I just send it always)
  ...
  */
  ...
  case STD_GET_DESCRIPTOR: {
    ...
    if (wValue == 0x100) // Return Device Descriptor
      ...
      AT91F_USB_SendData(pUdp, bosDescriptor,
                         MIN(sizeof(bosDescriptor), wLength));
  ...
  case STD_SET_CONFIGURATION:
     ...
     *   The LSB of wValue is the "Configuration Number". If this value is
     * non-zero, it should be the same number as defined in the Configuration
     * Descriptor; otherwise an error must have occurred. This device has only
     * one configuration and its Config Number is CONF_NB (= 1).
     */
    AT91F_USB_SendZlp(pUdp);
    ...
    */
    ...
    // enable endpoints
    pUdp->UDP_CSR[AT91C_EP_OUT] =
        (wValue) ? (AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_BULK_OUT) : 0;
    pUdp->UDP_CSR[AT91C_EP_IN] =
        (wValue) ? (AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_BULK_IN) : 0;
    pUdp->UDP_CSR[AT91C_EP_NOTIFY] =
        (wValue) ? (AT91C_UDP_EPEDS | AT91C_UDP_EPTYPE_INT_IN) : 0;
    break;
  case STD_GET_CONFIGURATION:
    AT91F_USB_SendData(pUdp, (char *)&(btConfiguration),
  ...
  case STD_SET_FEATURE_ENDPOINT:
    wIndex &= 0x0F;
    if ((wValue == 0) && (wIndex >= AT91C_EP_OUT) &&
        (wIndex <= AT91C_EP_NOTIFY)) {
