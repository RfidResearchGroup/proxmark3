...
extern uint32_t _bootrom_start[], _bootrom_end[], _flash_start[], _flash_end[],
    _osimage_entry[], __bss_start__[], __bss_end__[];

static int reply_old(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
...
static void ConfigClocks(void) {
  ...
  AT91C_BASE_PMC->PMC_PCER = (1 << AT91C_ID_PIOA) | (1 << AT91C_ID_ADC) |
                             (1 << AT91C_ID_SPI) | (1 << AT91C_ID_SSC) |
                             (1 << AT91C_ID_PWMC) | (1 << AT91C_ID_UDP);
...
static void UsbPacketReceived(uint8_t *packet) {
  ...
  case CMD_READ_MEM_DOWNLOAD: {
    ...
    bool raw_address_mode =
        ((flags & READ_MEM_DOWNLOAD_FLAG_RAW) == READ_MEM_DOWNLOAD_FLAG_RAW);
    if (!raw_address_mode) {
    ...
    if (isok) {
      for (size_t pos = 0; pos < count; pos += PM3_CMD_DATA_SIZE) {
        ...
        isok = 0 == reply_old(CMD_READ_MEM_DOWNLOADED, pos, len, 0,
                              &base[offset + pos], len);
        if (!isok)
          break;
      }
  ...
  case CMD_FINISH_WRITE: {
    ...
    if (c->arg[1] == 0xff && c->arg[2] == 0x1fd) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        int offset = 0;
        uint32_t page_n =
            (flash_address - (uint32_t)_flash_start) / AT91C_IFLASH_PAGE_SIZE;
        if (page_n >= AT91C_IFLASH_NB_OF_PAGES / 2) {
          page_n -= AT91C_IFLASH_NB_OF_PAGES / 2;
          efc_bank = AT91C_BASE_EFC1;
          // We need to offset the writes or it will not fill the correct bank
          // write buffer.
          offset = (AT91C_IFLASH_NB_OF_PAGES / 2) * AT91C_IFLASH_PAGE_SIZE /
                   sizeof(uint32_t);
        }
        ...
        /* Check that the address that we are supposed to write to is within our
         * allowed region */
        if (((flash_address + AT91C_IFLASH_PAGE_SIZE - 1) >= end_addr) ||
  ...
  case CMD_START_FLASH: {
    ...
    /* Only allow command if the bootrom is unlocked, or the parameters are
     * outside of the protected bootrom area. In any case they must be within
     * the flash area.
     */
    if ((bootrom_unlocked || ((cmd_start >= (uint32_t)_bootrom_end) ||
                              (cmd_end < (uint32_t)_bootrom_start))) &&
...
#ifdef WITH_FLASH
  if (FlashInit()) { // checks for existence of flash also ... OK because
                     // bootrom was built for devices with flash
...
void BootROM(void) {
  ...
  AT91C_BASE_PIOA->PIO_PPUDR =
      GPIO_USB_PU | GPIO_LED_A | GPIO_LED_B | GPIO_LED_C | GPIO_LED_D |
      GPIO_FPGA_DIN | GPIO_FPGA_DOUT | GPIO_FPGA_CCLK | GPIO_FPGA_NINIT |
      GPIO_FPGA_NPROGRAM | GPIO_FPGA_DONE | GPIO_MUXSEL_HIPKD |
      GPIO_MUXSEL_HIRAW | GPIO_MUXSEL_LOPKD | GPIO_MUXSEL_LORAW | GPIO_RELAY |
      GPIO_NVDD_ON;
  // (and add GPIO_FPGA_ON)
  // These pins are outputs
  AT91C_BASE_PIOA->PIO_OER = GPIO_LED_A | GPIO_LED_B | GPIO_LED_C | GPIO_LED_D |
                             GPIO_RELAY | GPIO_NVDD_ON;
  ...
  AT91C_BASE_EFC0->EFC_FMR =
      AT91C_MC_FWS_1FWS | MC_FLASH_MODE_MASTER_CLK_IN_MHZ(48);
  ...
  case AT91C_RSTC_RSTTYP_USER:
    /* In these cases the g_common_area in RAM should be ok, retain it if it's
     * there */
  ...
  if ((g_common_area.command == COMMON_AREA_COMMAND_ENTER_FLASH_MODE) ||
    ...
    g_common_area.flags.button_pressed = 0;
    // jump to Flash address of the osimage entry point (LSBit set for thumb
    // mode)
