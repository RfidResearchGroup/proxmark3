...
void mck_from_slck_to_pll(void) {
  ...
  // UPDATE:
  // we observed on 10% of the devices very wrong initial slow clock RC TIA
  // measures. Bumping delay to 16 helps fixing the issue even on the most
  // screwed RC.

  // enable main oscillator and set startup delay
  AT91C_BASE_PMC->PMC_MOR = AT91C_CKGR_MOSCEN | PMC_MAIN_OSC_STARTUP_DELAY(16);
