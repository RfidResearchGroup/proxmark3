...
typedef struct {
  int8_t
      forceanticol; // 0:auto 1:force executing anticol 2:force skipping anticol
  int8_t forcebcc;  // 0:expect valid BCC 1:force using computed BCC 2:force
                    // using card BCC
...
// T55XX - This setup will allow for the 4 downlink modes "m" as well as other
// items if needed. Given the one struct we can then read/write to flash/client
// in one go.
typedef struct {
  t55xx_config_t m[4]; // mode
} t55xx_configurations_t;

// Capabilities struct to keep track of what functions was compiled in the
// device firmware
typedef struct {
  uint8_t version;
  uint32_t baudrate;
...
// that will be handled as argument of safety level [0..2] respectiveley normal
// / lazy / safe However as how design is, MOUNT and UNMOUNT only need/have lazy
// as safest level so a safe level will still execute a lazy version see
// spiffs.c for more about the normal/lazy/safety information)
#define CMD_SPIFFS_MOUNT 0x0130
#define CMD_SPIFFS_UNMOUNT 0x0131
#define CMD_SPIFFS_WRITE 0x0132

// We take +0x1000 when having a variant of similar function (todo : make it an
// argument!)
...
// Operation not supported by device    client/pm3: probably only on pm3 once
// client becomes universal
...
// Input / output error                 pm3:        error in client frame
// reception
#define PM3_EIO -8
// Buffer overflow                      client/pm3: specified buffer too large
// for the operation
#define PM3_EOVFLOW -9
// Software error                       client/pm3: e.g. error in parsing some
// data
#define PM3_ESOFT -10
// Flash error                          client/pm3: error in RDV4 Flash
// operation
#define PM3_EFLASH -11
// Memory allocation error              client:     error in memory allocation
// (maybe also for pm3 BigBuff?)
#define PM3_EMALLOC -12
// File error                           client:     error related to file access
// on host
#define PM3_EFILE -13
// Generic TTY error
#define PM3_ENOTTY -14
// Initialization error                 pm3:        error related to trying to
// initialize the pm3 / fpga for different operations
#define PM3_EINIT -15
// Expected a different answer error    client/pm3: error when expecting one
// answer and got another one
#define PM3_EWRONGANSWER -16
// Memory out-of-bounds error           client/pm3: error when a read/write is
// outside the expected array
#define PM3_EOUTOFBOUND -17
// exchange with card error             client/pm3: error when cant get answer
// from card or got an incorrect answer
...
// execute pm3 cmd failed               client/pm3: when one of our pm3 cmd
// tries and fails. opposite from PM3_SUCCESS
#define PM3_EFAILED -21
// partial success                      client/pm3: when trying to dump a tag
// and fails on some blocks.  Partial dump.
#define PM3_EPARTIAL -22
// tearoff occurred                      client/pm3: when a tearoff hook was
// called and a tearoff actually happened
#define PM3_ETEAROFF -23

// Got bad CRC                          client/pm3: error in transfer of data,
// crc mismatch.
#define PM3_ECRC -24

// STATIC Nonce detect                  pm3:  when collecting nonces for
// hardnested
#define PM3_ESTATIC_NONCE -25

// No PACS data                         pm3:  when using HID SAM to retried PACS
// data
...
// No data                              pm3:        no data available, no host
// frame available (not really an error)
#define PM3_ENODATA -98
// Quit program                         client:     reserved, order to quit the
// program
...
#define BL_MAKE_VERSION(major, minor, patch)                                   \
