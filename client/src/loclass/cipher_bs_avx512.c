...
void doMAC_brute_match512(const uint64_t y_bits_bs[96 * BS512_WORDS],
    ...
    for (int tick = 0; tick < 32; tick++) {
        ...
        //mac_match = _mm512_andnot_si512(diff, mac_match); //works fine but throws errors with some GCC version (this is a GCC issue not a code issue)
        mac_match = _mm512_maskz_andnot_epi32(0xFFFF, diff, mac_match); //Performs as the above after compilation, but doesn't trigger GCC errors
