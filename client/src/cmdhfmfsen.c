...
int fm11_collect_nonces(const uint8_t *key, iso14a_card_select_t *card, iso14a_fm11rf08s_nonces_with_data_t *nonces) {
    ...
    clearCommandBuffer();
    SendCommandMIX(CMD_HF_MIFARE_ACQ_STATIC_ENCRYPTED_NONCES, 1, 0, MF_KEY_A, key, MIFARE_KEY_SIZE);
    if (WaitForResponseTimeout(CMD_ACK, &resp, 2500) == false) {
...
#define BSET_TEST(bs, s)  (((bs)[(s) >> 6] >>  ((s) & 63)) & 1)
    ...
    for (uint32_t i = 0; i < a->count; i++) { BSET_SET(bset_a, a->data[i].seed); }
    for (uint32_t i = 0; i < b->count; i++) { BSET_SET(bset_b, b->data[i].seed); }
...
static int fm11_prioritize_defaults(fm11_keylist_t *list) {
    if (list == NULL || list->count == 0) {
        return PM3_SUCCESS;
    }
    uint32_t dst = 0;
    for (uint32_t d = 0; d < ARRAYLEN(g_mifare_default_keys); d++) {
        for (uint32_t i = dst; i < list->count; i++) {
            if (list->data[i].key == g_mifare_default_keys[d]) {
...
static uint32_t fm11_accept_found_key(const iso14a_fm11rf08s_nonces_with_data_t *nonces,
    ...
    if (nt_a == nt_b && found_key[sec][key_type ^ 1] == false) {
        keys_found[sec][key_type ^ 1] = key;
        found_key[sec][key_type ^ 1] = true;
        accepted++;
    }

    for (uint8_t s = 0; s < FM11RF08S_SECTORS; s++) {
        uint32_t s_nt_a = fm11_bytes_to_u32(nonces->nt[s][0]);
        uint32_t s_nt_b = fm11_bytes_to_u32(nonces->nt[s][1]);
        for (uint8_t kt = 0; kt < 2; kt++) {
            if (found_key[s][kt] || candidates[s][kt].count != 1 || fm11_keylist_has_key(&candidates[s][kt], key) == false) {
                continue;
            }
            keys_found[s][kt] = key;
            found_key[s][kt] = true;
            accepted++;
            if (s_nt_a == s_nt_b && found_key[s][kt ^ 1] == false) {
                keys_found[s][kt ^ 1] = key;
                found_key[s][kt ^ 1] = true;
                accepted++;
            }
        }
    }
    ...
    for (uint8_t sec = 0; sec < FM11RF08S_SECTORS; sec++) {
        ...
        if (nt_a == nt_b) {
            if (found_key[sec][0] && found_key[sec][1] == false) {
                keys_found[sec][1] = keys_found[sec][0];
                found_key[sec][1] = true;
                derived++;
            } else if (found_key[sec][1] && found_key[sec][0] == false) {
                keys_found[sec][0] = keys_found[sec][1];
                found_key[sec][0] = true;
                derived++;
            }
            continue;
        }
        ...
        if (found_key[sec][0] && found_key[sec][1] == false) {
            ...
            if (fm11_derive_pair_key(nt_a, keys_found[sec][0], nt_b, &candidates[sec][1], &key)) {
        ...
        if (found_key[sec][1] && found_key[sec][0] == false) {
            ...
            if (fm11_derive_pair_key(nt_b, keys_found[sec][1], nt_a, &candidates[sec][0], &key)) {
    ...
    for (uint8_t sec = 0; sec < FM11RF08S_SECTORS; sec++) {
        for (uint8_t kt = 0; kt < 2; kt++) {
            if (found_key[sec][kt] == false && candidates[sec][kt].count == 1) {
                ...
                uint32_t nt_a = fm11_bytes_to_u32(nonces->nt[sec][0]);
                uint32_t nt_b = fm11_bytes_to_u32(nonces->nt[sec][1]);
                if (nt_a == nt_b && found_key[sec][kt ^ 1] == false) {
                    keys_found[sec][kt ^ 1] = keys_found[sec][kt];
                    found_key[sec][kt ^ 1] = true;
                    promoted++;
                }
            }
        }
...
static uint32_t fm11_offline_fixpoint(const iso14a_fm11rf08s_nonces_with_data_t *nonces,
    ...
    for (;;) {
        ...
        changed += fm11_promote_singletons(nonces, candidates, keys_found, found_key);
        changed += fm11_propagate_exact_reused_keys(candidates, keys_found, found_key);
        uint32_t r = 0;
        changed += fm11_try_derive_all(nonces, candidates, keys_found, found_key, &r);
        total_reduced += r;
...
int CmdHF14AMfSEN(const char *Cmd) {
    ...
    if (active_key == NULL) {
        PrintAndLogEx(WARNING, "Unable to collect FM11RF08S nonce evidence with known backdoor keys");
        return PM3_ESOFT;
    }
    ...
    char activity[80];
    snprintf(activity, sizeof(activity), "Loaded card UID %08X using backdoor key %s", uid, sprint_hex_inrow(active_key, MIFARE_KEY_SIZE));
    fm11_sen_progress(nonce_count, activity, 0, 0);
    if (keep_nonces) {
        char nonce_fn[FILE_PATH_SIZE] = {0};
        snprintf(nonce_fn, sizeof(nonce_fn), "hf-mf-%s-nonces_with_data", sprint_hex_inrow(card.uid, card.uidlen));
        pm3_save_fm11rf08s_nonces(nonce_fn, &nonces, true);
    ...
    for (uint8_t sec = 0; sec < FM11RF08S_SECTORS; sec++) {
        ...
        for (uint8_t kt = 0; kt < 2; kt++) {
            ...
            fm11_prioritize_defaults(&candidates[sec][kt]);
        }
        uint32_t nt_a = fm11_bytes_to_u32(nonces.nt[sec][0]);
        uint32_t nt_b = fm11_bytes_to_u32(nonces.nt[sec][1]);
    ...
    if (reader_mode) {
        ...
        if (accepted > 0) {
            ...
            uint32_t reduced = 0;
            uint32_t derived = fm11_offline_fixpoint(&nonces, candidates, keys_found, found_key, &reduced);
            if (reduced > 0) {
    ...
    snprintf(activity, sizeof(activity), "Prioritize defaults and %u duplicate candidates", duplicate_candidates);
    ...
    if (priority_count > 0 && priority_count <= FM11RF08S_MAX_GLOBAL_PRIORITY_KEYS) {
        snprintf(activity, sizeof(activity), "Batch-check %u default/duplicate keys with fchk", priority_count);
    ...
    if (offline_only) {
        ...
        retval = fm11_save_recovery_outputs(&card, &nonces, keys_found, found_key, no_oob);
    ...
    while (true) {
        ...
        if (res == PM3_SUCCESS) {
            ...
            reduced = 0;
            derived = fm11_offline_fixpoint(&nonces, candidates, keys_found, found_key, &reduced);
            if (reduced > 0) {
                snprintf(activity, sizeof(activity), "Reduced paired candidate sets by %u keys", reduced);
                fm11_sen_progress(nonce_count, activity, reduced, 0);
            }
