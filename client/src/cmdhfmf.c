...
*/
static int mfc_read_tag(iso14a_card_select_t *card, uint8_t carddata[256][16], uint8_t numSectors, char *keyFilename){
    ...
    if ((f = fopen(keyFilename, "rb")) == NULL) {
        PrintAndLogEx(WARNING, "Could not find key file " _YELLOW_("%s"), keyFilename);
        return PM3_EFILE;
    }
    ...
    for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
        ...
        if (bytes_read != MFKEY_SIZE) {
            PrintAndLogEx(ERR, "Key file reading error.");
            fclose(f);
            return PM3_EFILE;
        }
    }
        ...
        for (blockNo = 0; blockNo < mfNumBlocksPerSector(sectorNo); blockNo++) {
            ...
            if (received) {
                ...
                if (blockNo == mfNumBlocksPerSector(sectorNo) - 1) { // sector trailer. Fill in the keys.
                    for (int i=0; i<6; i++) {
                        data[i]    = (keyA[sectorNo][i]);
                        data[10+i] = (keyB[sectorNo][i]);
                    }
                }
                if (resp.status == PM3_SUCCESS) {
...
static int CmdHF14AMfDump(const char *Cmd) {
    ...
    int res=mfc_read_tag(&card, carddata, numSectors, keyFilename);
    if (res != PM3_SUCCESS)
        return res;
...
static int CmdHF14AMfView(const char *Cmd) {
    ...
    if(dumplen != 0) {
        ...
        block_cnt = MIN(MIFARE_1K_MAXBLOCK, (bytes_read / MFBLOCK_SIZE));
        if (bytes_read == 320)
            block_cnt = MIFARE_MINI_MAXBLOCK;
        else if (bytes_read == 2048)
            block_cnt = MIFARE_2K_MAXBLOCK;
        ...
        block_cnt = numSectors * 4 ;

        iso14a_card_select_t card ;
        int res=mfc_read_tag(&card, carddata, numSectors, NULL);
        if (res != PM3_SUCCESS)
            return res;

        // flatten carddata into dump
        dump=(uint8_t *)carddata;
    }
    ...
    if (sector > -1) {
        ...
        uint16_t mad[7 + 8 + 8 + 8 + 8] = {0};
        size_t madlen = 0;
        int res = MADDecode(dump, NULL, mad, &madlen, false);
        if (res != PM3_SUCCESS) {
            PrintAndLogEx(ERR, "can't decode MAD");
            return res;
        }
        ...
        // allocate memory
        UDATA d;
        d.bytes = calloc(card_size, sizeof(uint8_t));
        if (d.bytes == NULL) {
            return PM3_EMALLOC;
        }
        uint16_t dlen = 0;
