...
static int usage_hf14_autopwn(void) {
    ...
    PrintAndLogEx(NORMAL, "      hf mf autopwn * 1 f dictionaries/mfc_default_keys                      -- attack a mifare classic card (size 1K) with the default dictionary");
    ...
    PrintAndLogEx(NORMAL, "      hf mf autopwn k 0 A FFFFFFFFFFFF * 1 f dictionaries/mfc_default_keys   -- this command combines the two above (reduce the need for nested / hardnested attacks, by using a dictionary)");
    return 0;
}
...
static int CmdHF14AMfAutoPWN(const char *Cmd) {
    ...
    // Get the card prng type (weak=true / hard=false)
    prng_type = detect_classic_prng();

    // Print operating parameters
    if (verbose) {
        PrintAndLogEx(INFO, "[  SETTINGS  ] Card sectors .. " _YELLOW_("%d"), sectors_cnt);
