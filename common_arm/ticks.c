...
void StartTickCount(void) {
  // This timer is based on the slow clock. The slow clock frequency is between
  // 22kHz and 40kHz. We can determine the actual slow clock frequency by
  // looking at the Main Clock Frequency Register.
  while ((AT91C_BASE_PMC->PMC_MCFR & AT91C_CKGR_MAINRDY) == 0)
    ; // Wait for MAINF value to become available...
  uint16_t mainf = AT91C_BASE_PMC->PMC_MCFR &
                   AT91C_CKGR_MAINF; // Get # main clocks within 16 slow clocks
  // set RealTimeCounter divider to count at 1kHz, should be 32 if RC is exactly
  // at 32kHz:
  AT91C_BASE_RTTC->RTTC_RTMR =
      AT91C_RTTC_RTTRST |
      ((((MAINCK / 1000 * 16) + (mainf / 2)) / mainf) & AT91C_RTTC_RTPRES);
 ...
 */
uint32_t RAMFUNC GetTickCount(void) { return AT91C_BASE_RTTC->RTTC_RTVR; }
...
void StartCountSspClk(void) {
  AT91C_BASE_PMC->PMC_PCER |= (1 << AT91C_ID_TC0) | (1 << AT91C_ID_TC1) |
                              (1 << AT91C_ID_TC2); // Enable Clock to all timers
  AT91C_BASE_TCB->TCB_BMR = AT91C_TCB_TC0XC0S_TIOA1    // XC0 Clock = TIOA1
                            | AT91C_TCB_TC1XC1S_NONE   // XC1 Clock = none
                            | AT91C_TCB_TC2XC2S_TIOA0; // XC2 Clock = TIOA0

  // configure TC1 to create a short pulse on TIOA1 when a rising edge on TIOB1
  // (= ssp_clk from FPGA) occurs:
  AT91C_BASE_TC1->TC_CCR = AT91C_TC_CLKDIS; // disable TC1
  AT91C_BASE_TC1->TC_CMR =
      AT91C_TC_CLKS_TIMER_DIV1_CLOCK // TC1 Clock = MCK(48MHz)/2 = 24MHz
      | AT91C_TC_CPCSTOP             // Stop clock on RC compare
      | AT91C_TC_EEVTEDG_RISING      // Trigger on rising edge of Event
      | AT91C_TC_EEVT_TIOB   // Event-Source: TIOB1 (= ssp_clk from FPGA =
                             // 13,56MHz/16)
  ...
  // use TC2 to count TIOA0 pulses (giving us a 32bit counter (TC0/TC2) clocked
  // by ssp_clk)
  ...
  AT91C_BASE_TC0->TC_CCR =
      AT91C_TC_CLKEN | AT91C_TC_SWTRG; // enable and reset TC0
  AT91C_BASE_TC1->TC_CCR =
      AT91C_TC_CLKEN | AT91C_TC_SWTRG; // enable and reset TC1
  AT91C_BASE_TC2->TC_CCR =
      AT91C_TC_CLKEN | AT91C_TC_SWTRG; // enable and reset TC2

  //
  // synchronize the counter with the ssp_frame signal.
  // Note: FPGA must be in a FPGA mode with SSC transfer, otherwise SSC_FRAME
  // and SSC_CLK signals would not be present
  //
  while (AT91C_BASE_PIOA->PIO_PDSR & GPIO_SSC_FRAME)
  ...
  if ((AT91C_BASE_SSC->SSC_RFMR & SSC_FRAME_MODE_BITS_IN_WORD(32)) ==
      SSC_FRAME_MODE_BITS_IN_WORD(16)) { // 16bit frame
    while (AT91C_BASE_PIOA->PIO_PDSR & GPIO_SSC_CLK)
      ; // wait for ssp_clk to go low;
    while (!(AT91C_BASE_PIOA->PIO_PDSR & GPIO_SSC_CLK))
  ...
  // note: up to now two ssp_clk rising edges have passed since the rising edge
  // of ssp_frame it is now safe to assert a sync signal. This sets all timers
  // to 0 on next active clock edge
  AT91C_BASE_TCB->TCB_BCR =
      1; // assert Sync (set all timers to 0 on next active clock edge)
  // at the next (3rd) ssp_clk rising edge, TC1 will be reset (and not generate
  // a clock signal to TC0) at the next (4th) ssp_clk rising edge, TC0 (the low
  // word of our counter) will be reset. From now on, whenever the last three
  // bits of our counter go 0, we can be sure to be in the middle of a frame
  // transfer. (just started with the transfer of the 4th Bit).

  // The high word of the counter (TC2) will not reset until the low word (TC0)
  // overflows. Therefore need to wait quite some time before we can use the
  // counter.
...
void WaitMS(uint32_t ms) { WaitTicks((ms & 0x1FFFFF) * 1500); }
...
void StartCountUS(void) {
  ...
  AT91C_BASE_TCB->TCB_BMR =
      AT91C_TCB_TC0XC0S_NONE | AT91C_TCB_TC1XC1S_TIOA0 | AT91C_TCB_TC2XC2S_NONE;
...
uint32_t RAMFUNC GetCountUS(void) {
  // return (AT91C_BASE_TC1->TC_CV * 0x8000) + ((AT91C_BASE_TC0->TC_CV / 15) *
  // 10);
  //   By suggestion from PwPiwi,
  //   http://www.proxmark.org/forum/viewtopic.php?pid=17548#p17548
  return ((uint32_t)AT91C_BASE_TC1->TC_CV) * 0x8000 +
         (((uint32_t)AT91C_BASE_TC0->TC_CV) * 2) / 3;
}
...
void StartTicks(void) {
  ...
  AT91C_BASE_TC0->TC_CMR =
      AT91C_TC_CLKS_TIMER_DIV3_CLOCK | // MCK(48MHz) / 32
      AT91C_TC_WAVE | AT91C_TC_WAVESEL_UP_AUTO |
  ...
  while (AT91C_BASE_TC0->TC_CV < 2)
    ; // and has started (TC_CV > TC_RA, now TC1 is cleared)
...
// 1us = 1.5ticks.
void WaitUS(uint32_t us) { WaitTicks((us & 0x3FFFFFFF) * 3 / 2); }
