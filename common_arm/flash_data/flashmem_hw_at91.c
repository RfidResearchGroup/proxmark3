...
static uint16_t FlashSendByte(uint32_t data) {
...
static uint16_t FlashSendLastByte(uint32_t data) {
    return FlashSendByte(data | AT91C_SPI_LASTXFER);
}

#ifndef AS_BOOTROM

// send address after R/W cmd.
static void Flash_TransferAddress(uint32_t address) {
