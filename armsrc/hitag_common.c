...
uint16_t timestamp_high = 0;    // Timer Counter 2 overflow count, ~47min
...
void hitag_setup_fpga(bool ledcontrol) {
    StopTicks();
...
// Reader functions
void hitag_reader_send_bit(int bit, bool ledcontrol) {
                ...
                if (sof_received == false && *rxlen >= sof_bits) {
                    ...
                    if ((rx[0] >> (8 - sof_bits)) != ((1 << sof_bits) - 1)) {
                        if (sof_bits == 4) {
                            ...
                            if ((rx[0] & 0xE0) != 0xC0) {
                                Dbprintf("Warning, SOF is invalid rx[0]: 0x%02X", rx[0]);
                            }
                        } else {
                            DbpString("Warning, not all bits of SOF are 1");
                        }
                    }
                    ...
                    *rxlen -= sof_bits;
                    ...
                    DBG Dbprintf("rxlen: %d rx[0]: 0x%02X", *rxlen, rx[0]);
                    sof_received = true;
                }
            }
        }
...
void hitag_tag_send_bit(int bit, MOD modulation, bool ledcontrol) {
    if (ledcontrol) LED_A_ON();
